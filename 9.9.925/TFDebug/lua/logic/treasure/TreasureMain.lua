...
function TreasureMain:initUI( ui )
    ...
    self.txt_num_score = {}
    local txt_num_score1 = TFDirector:getChildByPath(btn_treasure1,'txt_num')
    local txt_num_score10 = TFDirector:getChildByPath(btn_treasure10,'txt_num')
    local txt_num_score30 = TFDirector:getChildByPath(btn_treasure30,'txt_num')
    table.insert(self.txt_num_score,txt_num_score1)
    table.insert(self.txt_num_score,txt_num_score10)
    table.insert(self.txt_num_score,txt_num_score30)
    ...
    self.btn_history = TFDirector:getChildByPath(ui,'btn_title1')
    self.txt_count = TFDirector:getChildByPath(ui,'txt_count')
    -- self.loading_bar = TFDirector:getChildByPath(ui,'loadingbar_1')
    
    local txt_shengyu = TFDirector:getChildByPath(ui,'txt_shengyu')
    self.txt_time = TFDirector:getChildByPath(ui,'txt_time')
    txt_shengyu:setPositionX(txt_shengyu:getPositionX() - 60)
    ...
    self.panel_node_item = createUIByLuaNew("lua.uiconfig_mango_new.treasure.IconXuanZhong")
    self.panel_node_item:retain()
    for i=1,19 do
        local node = self.panel_node_item:clone()
        if i > 16 then
            node:setScale(0.65)
    ...
    for i=1,5 do
        self.panel_boxes[i] = {}
        local boxNode = TFDirector:getChildByPath(ui, 'panel_box'..i)
        self.panel_boxes[i].rootNode = boxNode
        self.panel_boxes[i].btn = TFDirector:getChildByPath(boxNode, 'btn_baoxiang_s')
        self.panel_boxes[i].txt_num = TFDirector:getChildByPath(boxNode, 'txt_num')
        self.panel_boxes[i].panel = TFDirector:getChildByPath(boxNode, 'panel_list')
    end
    ...
    local panel_icon = TFDirector:getChildByPath(ui, 'panel_box1')
    self.panel_reward_item = TFDirector:getChildByPath(panel_icon, 'bg_icon')
...
function TreasureMain:loadData(count,configMessage,golds,props,boxCounts,boxIndex,round,boxRewardList,actTime,freeTimes,scoreRewards)
    print(count.."----------count---------------")
    ...
    --self.time = 5
    self:refresUI()
    ---elf.txt_count:setText(self.count)

    for i=1,#(self.txt_num_score) do
        self.txt_num_score[i]:setText(scoreRewards[i] or '')
    end
    self.highNodes ={}
    for i,v in ipairs(self.configList) do
        if (self.nodes[i]) then
            ...
            toastMessage(localizable.ActionError)
        end
    end
...
function TreasureMain:refresUI()
    ...
    for i=1,3 do
        ...
        local tmpGoods = self.golds[i]
        local commonReward = {}
        commonReward.type   = EnumDropType.GOODS
        commonReward.itemId = tonumber(tmpGoods[2])
        commonReward.number = tonumber(tmpGoods[3])
        local tool = BaseDataManager:getReward(commonReward)
        self.btn_props[i]:setVisible(true)
        if tool  then
            self.btn_props[i]:setTexture(tool.path) 
            self.btn_props[i]:setScale(0.4)
            self.btn_golds[i]:setText("1") 
            
            local tool_Have = BagManager:getItemById(tonumber(self.props[i]))
            if (tool_Have and tool_Have.num >= 1) then
                self.txt_numb_haves[i]:setVisible(true)
                self.txt_numb_haves[i]:setText(stringUtils.format(localizable.changetProLayer_have ,tool_Have.num))
            else
            ...
            self.btn_golds[i]:setText(tmpGoods[3])
            self.txt_numb_haves[i]:setVisible(false)
        end    
    end
...
function TreasureMain:runHighAction(rewardData)
...
function TreasureMain:nextItem(round ,round_now,index)   
    ...
    if self.m_currentItemIndex == self.m_targetNumber and round == round_now then
        if self.isSecondHigh == true then
            self.onceMulripleTimes = 0
            self.onceMulripleNum = 0
            self.isSecondHigh = false
            self:doMulripleOverAction()
        else
            self:doOverAction()
        end
        if self.updateTimerID then
   ...
   -- print("---%d---%d---%d",self.m_currentItemIndex,self.m_targetNumber,self.m_totalItemCount)
    ...
    if self.m_currentItemIndex  <= 0 then
        self.m_currentItemIndex = self.m_totalItemCount + self.m_currentItemIndex
        return true
    end
    return false
end

function TreasureMain:runSecondHighAction()
    --if self.m_currentItemIndex < self.m_targetNumber then--
        self.isSecondHigh = true
        self:highlightTargetItem(self.m_currentItemIndex)
        self.m_currentItemIndex = self.m_currentItemIndex - 1
        if self.updateTimerID then
                TFDirector:removeTimer(self.updateTimerID)
        ...
        self.updateTimerID = TFDirector:addTimer(30, -1, nil,
            function()
                if self.updateTimerID ~= nil then
                    local last = (round - round_now)*self.m_totalItemCount + self.m_currentItemIndex - self.m_targetNumber
                    if last < 8 then
                        need_time = need_time + 30
                        if need_time >= 200 then
                            if self:nextItem(round ,round_now,-1) then
...
function TreasureMain:initCell(node,configList)
    local nodeIndex = 1
    local mulriple = configList.mulriple
    if mulriple and mulriple > 1 then
        nodeIndex = 2
        if mulriple > 9 then
            nodeIndex = 3
        end
    ...
    local rootNode = nil 
    for i=1,3 do
        local panelNode = TFDirector:getChildByPath(node, 'panel_' .. i)
        if i == nodeIndex then
            rootNode = panelNode
    ...
    local txt_num = TFDirector:getChildByPath(rootNode, 'txt_num')
    if nodeIndex > 1 then
        txt_num:setText(mulriple .. '')
        return
    end

    local bg_icon = TFDirector:getChildByPath(node, 'bg_icon')
    local img_icon = TFDirector:getChildByPath(rootNode, 'img_icon')
    ...
    if newCardRoleData ~= nil then
        ...
        if item.resType == EnumDropType.GOODS then
            ...
            elseif newCardRoleData.type == EnumGameItemType.Piece or newCardRoleData.type == EnumGameItemType.ItemPiece then
                Public:addPieceImg(img_icon,newCardRoleData,true)
            else
...
function TreasureMain:doOverAction()
    ...
    local funcall = function()
        ...
        node:setTag(999)
        if self.m_secondTargetNumber ~= nil then
            self.m_targetNumber = self.m_secondTargetNumber
            self.m_secondTargetNumber = nil
            self:runSecondHighAction()
            return
        end        
        local callFuncN = function()
            self.bClick = true
        ...
        self.panel_1:timeOut(callFuncN)
    end
    node:Tween{
        {
            duration = 0.5,
...
function TreasureMain:doMulripleOverAction()
    local configItem = clone(self.configList[self.m_targetNumber])

    local data = TreasureManager:getReward()
    local rewardList = data.rewardList
    local totalNum = configItem.number
    local maxTimes = 1
    if #rewardList > 0 then
        local totalMulriple = rewardList[1].mulriple or 1
        maxTimes = totalMulriple
        if totalMulriple > 6 then
            maxTimes = 6
        end
        totalNum = rewardList[1].number
    end
    local addNum = math.floor(totalNum/maxTimes)
    if addNum == 0 then
        addNum = 1
    end
    local leftNum = totalNum - self.onceMulripleNum
    local leftTimes = maxTimes - self.onceMulripleTimes
    if leftNum > 0 and leftTimes > 0 and (leftNum) % (leftTimes) == 0 then
        addNum = leftNum / leftTimes
    end
    self.onceMulripleTimes = self.onceMulripleTimes + 1
    self.onceMulripleNum =  addNum + self.onceMulripleNum
    if self.onceMulripleNum >= totalNum then
        self.onceMulripleNum = totalNum
    end
    configItem.number = self.onceMulripleNum

    local node = self.panel_node_item:clone()
    node:setScale(0.5)
    self:initCell(node,configItem)
    node:setPosition(ccp(self.nodes[self.m_targetNumber]:getPosition()))
    ...
    local funcall = function ()
        ...
        if self.onceMulripleTimes < maxTimes then
            self:doMulripleOverAction()
            return
        end
        local callFuncN = function()
...
function TreasureMain.onBtnTreansure(btn)
    ...
    elseif tool and tool.num > 0 then --物品足够,直接扣除物品
        self:requestInfo(index,tool.num)
    elseif (not tool) then  --物品不足,提示购买
        ResGetWayManager:openListShow({20})
...
function TreasureMain.onBtnGetBox(sender)
    local self = sender.logic
    if self.boxIndex + 1 ~= sender.index then
        print(' cannt jump')
        return
    end 
    TreasureManager:requestBoxReward(sender.index)
end
...
function TreasureMain:removeUI()
    ...
    if self.panel_reward_item then
        self.panel_reward_item:release()
        self.panel_reward_item = nil
    end
    if self.panel_node_item then
        self.panel_node_item:release()
        self.panel_node_item = nil
    end
end
...
function TreasureMain:registerEvents()
    ...
    self.onOnceResult = function()
        ...
        if #data.rewardList == 1 then
            local random = data.index[1] or math.random(19)
            ...
            self.m_secondTargetNumber = data.index[2]
            self:runHighAction(data.rewardList[1])
        else
...
function TreasureMain:refreshRankList()
    ...
    if TreasureManager.myRank.rank and TreasureManager.myRank.rank > 0 and TreasureManager.myRank.rank <= 50 then
        rank_txt:setText(TreasureManager.myRank.rank)
    else
        --rank_txt:setText("未入榜")
        rank_txt:setText(localizable.shalu_info_txt1)
    end
    local score = TreasureManager.myRank.score or 0
...
function TreasureMain.onHistoryClick(sender)
    local layer  = require("lua.logic.treasure.TreasureRecordLayer"):new()
    AlertManager:addLayer(layer,AlertManager.BLOCK_AND_GRAY,AlertManager.TWEEN_1)
    AlertManager:show()
end

function TreasureMain.box_numberOfCellsInTableView(table)
    local self = table.logic
    local index = table.index
    local data = self.boxRewardList[index] or {}
    if data and data.boxReward then
        return #(data.boxReward)
    end
    return 0
end

function TreasureMain.box_cellSizeForTable(table,idx)
    return 56,56
end

function TreasureMain.box_tableCellAtIndex(table, idx)
    local cell = table:dequeueCell()
    local self = table.logic
    if nil == cell then
        cell = TFTableViewCell:create()
        ...
        panel_item:setPosition(ccp(3,3))
    ...
    local data = self.boxRewardList[index] or {}
    if data and data.boxReward then
        rewardInfo = data.boxReward[idx+1]
    end
...
function TreasureMain:loadBoxCellInfo(panel_item,info)
...
function TreasureMain:loadBoxReward()
    self:refreshBoxInfo()
    for k,v in pairs(self.boxRewardList) do
        local panel = self.panel_boxes[k].panel
        ...
        tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, TreasureMain.box_cellSizeForTable)
        tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, TreasureMain.box_tableCellAtIndex)
        tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, TreasureMain.box_numberOfCellsInTableView)
        tableView = tableView
        tableView.logic = self
        tableView.index = k
        tableView:setPosition(ccp(0,0))
        self.panel_boxes[k].tableView = tableView
        panel:addChild(tableView)
        tableView:reloadData()
    end
end

function TreasureMain:refreshBoxInfo()
    local listCount = #self.boxRewardList
    for i=1,listCount do
        self.panel_boxes[i].rootNode:setVisible(true)
        if i > self.boxIndex then
            local count = self.boxCounts[i] +  self.round * self.boxCounts[5]
            self.panel_boxes[i].txt_num:setText( stringUtils.format(localizable.treasureMain_text3 ,count))                      
            self.panel_boxes[i].btn.count = count
        else
            local count = self.boxCounts[i] +  (self.round + 1)* self.boxCounts[5]
            self.panel_boxes[i].txt_num:setText(stringUtils.format(localizable.treasureMain_text3 ,count)) 
            self.panel_boxes[i].btn.count = count
        end   
    end
    ...
    self.new_btn_boxs = {}
    for i=self.boxIndex + 1,listCount do
    ...
    for i=1,listCount do
        local posy = 157 - ( i - 1 ) * 87
        self.new_btn_boxs[i].rootNode:setPositionY(posy)    
    end

    self:brushBoxRedPoint()
end

function TreasureMain:brushBoxRedPoint()
    local open_state = true 
