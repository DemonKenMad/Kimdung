...
function MissionDetailLayer:refreshUI()
    ...
    local FightMgr = require('lua.gamedata.FightManager')
    if (mission.mapid == 1 and mission.stageid == 6) then
        FightMgr.isCheckPoint1_6 = true
    else 
        FightMgr.isCheckPoint1_6 = false
    end
    ...
    local map = MissionManager:getMapById(mission.difficulty,mission.mapid)
...
function MissionDetailLayer:refreshQuickPass()
    ...
    if mission.difficulty == MissionManager.DIFFICULTY0  then
        if mission.type == MissionManager.TYPE_COMMON then
            self.maxChallengeCount = mission.maxChallengeCount
        else
...
function MissionDetailLayer:onClickHandleForTrustLayer()
    self.onAttackClickHandle(self.btn_attack)
end
...
function MissionDetailLayer.onAttackClickHandle(sender)
    ...
    elseif sender == self.btn_quick3 then
        ...
        if (( MissionManager.openSystemList[EnumUnlockSystem.multipleSweep] == nil or MissionManager.openSystemList[EnumUnlockSystem.multipleSweep] ~= true) and MainPlayer:getVipLevel() < openVip) then
...
function MissionDetailLayer:drawCell(cell, cellIndex)
    local totalNum = self.rewardList:length()
