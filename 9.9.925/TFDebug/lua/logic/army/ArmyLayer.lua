...
ArmyLayer.LIST_ITEM_HEIGHT = 173; 
...
function ArmyLayer:ctor(data)
    ...
    self.isFromOnlySelfPowerTeam = false
    self:init("lua.uiconfig_mango_new.role.ArmyLayer");
end

function ArmyLayer:onShow()
    print("ArmyLayer:onShow()11111111111111111111111")
...
function ArmyLayer:updateStrategyBaseMsg()
    ...
    self.txt_power:setText(StrategyManager:getPower(self.isFromOnlySelfPowerTeam));

    if (self.isFromOnlySelfPowerTeam and CardRoleManager.isOnlySelfPower) then
        self.assistFightView:setVisible(false)
        for i=1, 9 do 
            local img_zw = TFDirector:getChildByPath(self.zhenfaNode, "img_zf"..i)
            img_zw:setVisible(false)
        end
        self.txt_closeAssist:setVisible(true)
    else
        self.assistFightView:setVisible(true)
        self.txt_closeAssist:setVisible(false)
    end
end
...
function ArmyLayer:initUI(ui)
    ...
    self.txt_closeAssist = TFDirector:getChildByPath(ui, 'txt_miaoshu');
end
...
function ArmyLayer.cellTouchMovedHandle(cell)
    ...
    if self.guideRolePos > 0 then 
        local cellRole = CardRoleManager:getRoleByGmid(cell.gmId)
        local pos = tonumber(cellRole.pos) or 0 
        if cellRole ~= nil and pos > 0 then
            return
        end
    end
...
function ArmyLayer:createSelectCussor(cell,pos)
    ...
    if (not role) then
        local id = cell.gmId or ""
        toastMessage(localizable.common_data_error.."gmId:  "..id)
        return 
    end
...
function ArmyLayer.onSkillBtnClick( btn )
...
--设置减除所有团队增益
function ArmyLayer:setSelfPower(onlyselfAdd)
    self.isFromOnlySelfPowerTeam = onlyselfAdd
    local isShow = self.isFromOnlySelfPowerTeam and CardRoleManager.isOnlySelfPower
    Public:addZhenFaIconForArmy(self.zhenfaNode, self.currLineUpType, not isShow)
