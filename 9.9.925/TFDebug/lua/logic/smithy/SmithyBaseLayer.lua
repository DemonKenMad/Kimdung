...
local tabPressedPaths = {
	...
	"ui_new/smithy/tab_upgrade_pressed.png",--装备升阶
...
local tabNormalPaths = {
	...
	"ui_new/smithy/tab_upgrade.png",--装备升阶6
...
local layerDeclear = {
	...
	"lua.logic.smithy.EquipmentUpgradeLayer",--装备升阶
...
function SmithyBaseLayer:initUI(ui)
    ...
    local baseX = 61 
    local perAdd = 110
    local lockList = {[5] = EnumVersionLockType.SmithyRecast_Lock, [6] = EnumVersionLockType.EquipmentUpgrade_Lock}
	for i=1,SmithyBaseLayer.kTabNumber do
		...
		self.btn_tab[i]:setPositionX(baseX)
		local lockId = lockList[i]
		if lockId then
			local verLock = VersionLockData:objectByID(lockId)
			if verLock and verLock.open == 0 then
				self.btn_tab[i]:setVisible(false)
			end
		end
		

		if self.btn_tab[i]:isVisible() == true then
			baseX = baseX + perAdd
		end
	...
	self.scrollViewTitle = TFDirector:getChildByPath(ui, "ScrollViewTitle")
	-- self.scrollViewTitle:scrollToLeft()
end

function SmithyBaseLayer:setEquipGmId(gmId)
	self.gmId = gmId
	local equipInfo = EquipmentManager:getEquipByGmid(gmId)
	if equipInfo then
		self.gmId = equipInfo.gmId
		local tmpLevel = equipInfo.level + 1
		if (equipInfo.level >= MainPlayer:getMaxIntensifyLevel()) then
			tmpLevel = equipInfo.level
		end
		local coin = IntensifyData:getConsumeByIntensifyLevel(tmpLevel, equipInfo.quality)
	    local bTemp = MainPlayer:isEnoughCoin(coin, false)
	    local redpoint = false
	    if bTemp and equipInfo.level < MainPlayer:getMaxIntensifyLevel() then
	    	redpoint = true
	    end
	    self:RedBtn_Tab(1,"IsCanIntensify",redpoint)
	end
	local bxiangqian = KaojiaoManager:checkEquipXiangqianByEquipId(self.gmId,false)
	self:RedBtn_Tab(7,"IsCanXiangQian",bxiangqian)

	self.operationLayer[self.selectedTab]:setVisible(true)
	if self.selectedTab ~= SmithyBaseLayer.kTabNumber then
...
function SmithyBaseLayer:refreshTabButton()
	...
	CommonManager:updateWidgetState(self.btn_tab[2],EquipmentManager.Function_StarUp,false,ccp(-10,0))
	CommonManager:updateWidgetState(self.btn_tab[3],EquipmentManager.Function_Parctice,false,ccp(-10,0))
	CommonManager:updateWidgetState(self.btn_tab[5],EquipmentManager.Function_Recast,false,ccp(-10,0))
	CommonManager:updateWidgetState(self.btn_tab[6],EquipmentManager.Function_UpGrade,false,ccp(-10,0))
	CommonManager:updateWidgetState(self.btn_tab[4],EquipmentManager.Function_Refining,EquipmentManager:isHaveNewRefinStone(),ccp(-10,0))
	local bxiangqian = KaojiaoManager:checkEquipXiangqianByEquipId(self.gmId,false)
	CommonManager:updateWidgetState(self.btn_tab[7],EquipmentManager.Function_Gem_Mount,bxiangqian,ccp(-10,0))
	CommonManager:updateWidgetState(self.btn_tab[8],EquipmentManager.Function_Gem_Merge,EquipmentManager:isHaveGemEnough(),ccp(-10,0))
...
function SmithyBaseLayer.tabButtonClickHandle(sender)
	...
	for i = 1,#layerDeclear do
		if self.selectedTab ~= i and self.operationLayer[i] then
			...
			if i == SmithyBaseLayer.kTabNumber then
	...
	if not self.operationLayer[self.selectedTab] then
		...
		self.operationLayer[self.selectedTab].logic = self
	...
	if self.selectedTab == 7 and EquipmentManager:isHaveNewGem() then
		EquipmentManager:onIntoGemLayer()
		CommonManager:removeRedPoint(self.btn_tab[7])
	end
	...
	-- 	self.scrollViewTitle:scrollToLeft(0.2,false)
	-- end
	-- if self.selectedTab == 5 then
	-- 	self.scrollViewTitle:scrollToLeft(0.2,false)
	-- end
...
function SmithyBaseLayer:registerEvents()
    ...
    function onTouchBegin(widget,pos,offset)
    	local currentPage = self.operationLayer[self.selectedTab]
    	if self.selectedTab == 2 or self.selectedTab == 7 or self.selectedTab == 8 then
    ...
    function onTouchEnd(widget,pos)
    	if self.touchInTableView or self.selectedTab == SmithyBaseLayer.kTabNumber then
    		return
    	end
...
function SmithyBaseLayer:RedBtn_Tab(index,name,redpoint)
	if redpoint == true then
    	CommonManager:setRedPoint(self.btn_tab[index], true,name,ccp(-10,0))
    else
    	CommonManager:setRedPoint(self.btn_tab[index], false,name,ccp(-10,0))
    end
end
