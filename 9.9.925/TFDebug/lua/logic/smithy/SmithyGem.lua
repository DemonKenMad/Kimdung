...
function SmithyGem:updateRedPoint(bBrush)
    if self.logic then
        local bxiangqian,tXQInfo = KaojiaoManager:checkEquipXiangqianByEquipId(self.gmId,bBrush)
        self.logic:RedBtn_Tab(7,"IsCanXiangQian",bxiangqian)
        for i=1,EquipmentManager.kGemMergeTargetNum do
            if (tXQInfo and tXQInfo[i]) then
                CommonManager:setRedPoint(self.info_panel.imgGemNode[i], tXQInfo[i],"IsCanXiangQian",ccp(-10,-10))
            end
        end
    end
end
