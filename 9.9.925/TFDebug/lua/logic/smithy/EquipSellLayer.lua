...
function EquipSellLayer:refreshEquipList()
	...
	self.equipList = EquipmentManager:GetEquipByTypeAndUsed(self.selectedTab,false,nil,false);
...
function EquipSellLayer:calculationResult( gmid )
	...
	--装备升阶返回
	local equipUpGradeLevel = equip:getOrderLevel()
	local equipUpgradeExp = EquipmentThroughExpData:getTotalExpByQualityLevel(equip.quality, equipUpGradeLevel)
	equipUpgradeExp = equipUpgradeExp + equip:getCurrExp()
	local desId = ConstantData:objectByID("EquipmentThroughSoldItem").value
	local percent = ConstantData:objectByID("EquipmentThroughSoldNum").value/100
	local itemData = ItemData:objectByID(desId)
	local sigleExp = itemData.provide_exp

	local equipUpGradeNum = math.floor((equipUpgradeExp/sigleExp)*percent/100)

	resultTbl.equipUpGradeId = desId
	resultTbl.equipUpGradeNum = equipUpGradeNum
...
function EquipSellLayer:calculateReward()
    ...
    for k,v in pairs(self.selectIconTable) do
    	if v then
    		...
    		if tbl.equipUpGradeId ~= 0 and tbl.equipUpGradeNum ~= 0 then
    			goodsTbl[tbl.equipUpGradeId] = goodsTbl[tbl.equipUpGradeId] or 0
    			goodsTbl[tbl.equipUpGradeId] = goodsTbl[tbl.equipUpGradeId] + tbl.equipUpGradeNum
    		end
    	end
    end
