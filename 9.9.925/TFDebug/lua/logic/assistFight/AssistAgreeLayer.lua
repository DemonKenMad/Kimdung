...
function AssistAgreeLayer:initUI( ui )
    ...
    self.detailCurrAdd = {}
    self.detailCurrAdd[1] = TFDirector:getChildByPath(instruNode, "txt_shuxing1")
    self.detailCurrAdd[2] = TFDirector:getChildByPath(instruNode, "txt_shuxing3")
    self.detailNextAdd = {}
    self.detailNextAdd[1] = TFDirector:getChildByPath(instruNode, "txt_shuxing2")
    self.detailNextAdd[2] = TFDirector:getChildByPath(instruNode, "txt_shuxing4")
...
function AssistAgreeLayer:showIocnLevelNode(pos, isSelect)
    ...
    if iconData.LockState then
        ...
        if #iconData.attrTbl[1] == 1 then
            local attrIdx = tonumber(iconData.attrTbl[1][1])
        ...
        for j=1,5 do
            ...
            if iconData.level < j then
                ...
                -- if iconData.level > 5 then
                --     local exIndex = iconData.level - 5
                --     if exIndex >= j then
                --         starIcon:setTexture("ui_new/Zhuzhan/icon_qhlv_zi.png")                        
                --     end
                -- else
...
function AssistAgreeLayer:showIocnLevelDetails()
    ...
    --     end
    -- end
    local rankIndex = math.floor((tonumber(data.level) - 1) / 5) + 1
    local currRank  = math.floor((tonumber(data.level) - (rankIndex - 1) * 5))
    for j=1,5 do
        local starIcon = TFDirector:getChildByPath(self.detailStarNode, "icon_lv"..j)
        if data.level < j then
            starIcon:setVisible(false)
    ...
    for i=1,2 do
        if data.attrTbl[i] == nil then
            self.detailCurrAdd[i]:setVisible(false)
            self.detailNextAdd[i]:setVisible(false)
        else
            local currValue = math.floor(data.currBuf[i]/100)
            local nextValue = math.floor(data.nextBuf[i]/100)
            ...
            if #data.attrTbl[i] == 1 then
                local attrIdx = tonumber(data.attrTbl[i][1])
                ...
                local attrStr = nil
                for _,attrIdx in pairs(data.attrTbl[i]) do
                    if attrStr then
            ...
            self.detailCurrAdd[i]:setVisible(true)
            self.detailCurrAdd[i]:setText(currAdd)
            ...
            if data.level ~= qiheMaxLevel then
                ...
                self.detailNextAdd[i]:setVisible(true)
                self.detailNextAdd[i]:setText(nextAdd)
            end
        end
    end
    if data.level == qiheMaxLevel then
        self.detailNextName:setVisible(false)
        self.detailNextAdd[1]:setVisible(false)
        self.detailNextAdd[2]:setVisible(false)
        ...
        -- self.detailNextAdd[1]:setVisible(true)
        -- self.detailNextAdd[2]:setVisible(true)
        local str = stringUtils.format(localizable.assistAgreeLayer_text2, data.level + 1)
        self.detailNextName:setText(str)
...
function AssistAgreeLayer:achieveDataReady()
    ...
    for i=1,AchieveMax do
        ...
        LocalAchieveData[i].achieveDes = {}
        local achieveDes,achieveName = AssistFightManager:getAchieveTemplete(level)
        LocalAchieveData[i].achieveName = achieveName
        LocalAchieveData[i].achieveDes = achieveDes
        if currNum >= totalNum then
            achieveCount = achieveCount + 1
        end
    end
    print('self.currAchieveCount = ',self.currAchieveCount)
    print('achieveCount = ',achieveCount)
    local oldAchieveCount = self.currAchieveCount or achieveCount
...
function AssistAgreeLayer.qiheButtonClick( btn )
    ...
    for k,item in pairs(data.item) do
        ...
        if curr_num < item.num then
            table.insert(itemList,{item.id, EnumDropType.GOODS})
            --toastMessage(localizable.assistAgreeLayer_no_pro)
            --return
        end
    end
    if #itemList > 0 then
        if ResGetWayManager:openResGetLayerByList(itemList) == false then
            toastMessage(localizable.assistAgreeLayer_no_pro)
        end
        return
    end
...
function AssistAgreeLayer:cellInfoSet(cell, panel, idx)
    ...
    shuomingOff:setText(item.achieveDes)
    shuomingOn:setText(item.achieveDes)
