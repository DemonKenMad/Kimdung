...
function MapLayer:ctor(data)
	...
	if not TFFileUtil:existFile(mapName) then
		mapName = "fightmap/mission1.jpg"
	end
...
function MapLayer:GetMapID()
	...
	elseif fightType == 12 then --10:世界boss
		return 30
	elseif fightType == 39 then
		return HeroListManager:getMapId()
	elseif fightType == EnumFightType.HeroicBoss then
		return HeroListManager:getMapId()
	else
		return 1
	end
...
function MapLayer.GetPosByIndex(posIndex,roleId)
