...
function XiaKeZhuanHuanLayer:initUI(ui)
    ...
    ViewSizeCheck(TFDirector:getChildByPath(ui,'img_bg'))
    ...
    self.changeSuccess = function(event)       
        ...
        if data and data.result then
            if data.result ~= 0 then
                ...
                local skill1 = cardRole1.skillStarLevel
                local skill2 = cardRole2.skillStarLevel
                local bSet = true
                if (skill1 == 3) then
                    bSet = false
                    local supperInfoList = RoleTalentData:GetRoleSupperStarInfoByRoleId(cardRole2.id)
                    if (supperInfoList[3] and supperInfoList[3].star_lv == 3) then
                        bSet = true
                    end
                
                elseif (skill2 == 3) then
                    bSet = false
                    local supperInfoList = RoleTalentData:GetRoleSupperStarInfoByRoleId(cardRole1.id)
                    if (supperInfoList[3] and supperInfoList[3].star_lv == 3) then
                        bSet = true
                    end
                end
                if bSet and cardRole1.quality > 5 and cardRole2.quality > 5 then
                    cardRole2:setSkillStarLevel(skill1)
                    cardRole1:setSkillStarLevel(skill2)
                end
...
function XiaKeZhuanHuanLayer:getRoleList()
    ...
    for roleId in CardRoleManager.cardRoleList:iterator() do
        local card = CardRoleManager:getRoleById(roleId)
        local id1, id2 = -1, -1
        if self.selectRole1 then
        ...
        if (card.quality >= 4) and card.gmId ~= id1 and card.gmId ~= id2 and card.id ~= MainPlayer:getProfession() then
            --[[
            if id1 ~= -1 then
            ...
            role_list:pushBack(roleId)
        elseif card.id == MainPlayer:getProfession() and card.gmId ~= id1 and card.gmId ~= id2 and (card.quality > 4) then 
            role_list:pushBack(roleId)
        end       
    end
...
function XiaKeZhuanHuanLayer.sortFunc(cardRoleId1, cardRoleId2)
