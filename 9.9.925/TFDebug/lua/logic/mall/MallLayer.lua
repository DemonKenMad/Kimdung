...
function MallLayer:initUI(ui)
    ...
    self.MallInfoList = {
        ...
        {path = 'lua.logic.mall.XilingMallInfo',tap = 'ui_new/shop/btn_dfsd.png'},
        {path = 'lua.logic.mall.XinFaMallInfo',tap = 'ui_new/shop/btn_jxz.png'},
        {path = 'lua.logic.mall.HeroMallInfo',tap = 'ui_new/shop/btn_wwsc.png'},
        {path = 'lua.logic.mall.TianMingMallInfo',tap = 'ui_new/shop/btn_jpsd.png'},
        {path = 'lua.logic.mall.FengYunLuMallInfo',tap = 'ui_new/shop/btn_fyg.png'},
    }
    ...
    ViewSizeCheck(TFDirector:getChildByPath(ui, 'bgImg'))
    self.btnList = {}
    for i=1,5 do
    ...
    -- self:showInfo(EnumMallType.NormalMall,3)
end
...
function MallLayer:showInfo0(index,btnIdx)
    ...
    local index1 = self:mallTurnLeft(self.curInfoIndex,self.curInfoIndex)
    local index2 = self:mallTurnRight(self.curInfoIndex,self.curInfoIndex)
...
function MallLayer:mallTurnLeft(index0,index1)
    local index = index0
    index = index - 1
    if index <= 0 then
    ...
    if mallInfo:isOpen() == false then
        return self:mallTurnLeft(index,index1)
    else
        return index
    end
end

function MallLayer:mallTurnRight(index0,index1)
    local index = index0
    index = index + 1
    ...
    local mallInfo = self.MallInfoList[index].info
    if mallInfo:isOpen() == false then
        return self:mallTurnRight(index,index1)
    else
        return index
    end
end
...
function MallLayer:pageRightClickHandle()
    ...
    self.curInfoIndex = self:mallTurnRight(self.curInfoIndex,self.curInfoIndex)
    local tampBtn = {}
    for i=1,3 do
...
function MallLayer:pageLeftClickHandle()
    ...
    if self.curInfoIndex == nil then self.curInfoIndex = 1 end
    self.curInfoIndex = self:mallTurnLeft(self.curInfoIndex,self.curInfoIndex)
    local tampBtn = {}
    for i=1,3 do
        tampBtn[i] = self.btnTap[i]
    end
