...
function BloodFightArmyLayer:ctor(data)
    ...
    date_time.min  = 0
    date_time.sec  = 1
    self.tip_time = GetCorrectTime(date_time)
...
function BloodFightArmyLayer:refreshUI()
    ...
    if self.firstIn then
        self.firstIn = false
        self.btn_type_choice = self.btn_xiake
...
function BloodFightArmyLayer:_loadMercenaryNode(item_node,roleItem)
    ...
    local roleStar = ""
    if roleItem.starlevel and roleItem.starlevel > 0 then
        roleStar = roleStar .. " +" .. roleItem.starlevel
    end
...
function BloodFightArmyLayer.onSkillBtnClick( btn )
