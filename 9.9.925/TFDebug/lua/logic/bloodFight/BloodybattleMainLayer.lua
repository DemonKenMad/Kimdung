...
function BloodybattleMainLayer:initUI(ui)
    ...
    self.generalHead:setData(ModuleType.Bloodybattle,{HeadResType.COIN,HeadResType.SYCEE,HeadResType.PUSH_MAP}, true)
...
function BloodybattleMainLayer:onAttackCompeleteHandle(event)
    ...
    if (nowSection == 9) and MissionManager:isInTrustOnekey() then
        pageIndex = math.ceil(nowSection/8)
        print("pageIndex3 = ", pageIndex)
...
function BloodybattleMainLayer.onBtnOneKeyClick(sender)
    local self = sender.logic

    local currIndex = BloodFightManager:getMissionIndex()
    local total = BloodFightManager:getMissionTotalCount()

    if currIndex > total then
...
function BloodybattleMainLayer:onClickHandleForTrustLayer()
    ...
    self:setGuideBlockViisble(false)
    local player = BloodFightManager:getPlayer(currIndex)
    local status = BloodFightManager:getPlayerStaus(player.index);
