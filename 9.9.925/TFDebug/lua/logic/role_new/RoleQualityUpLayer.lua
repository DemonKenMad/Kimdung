...
function RoleQualityUpLayer:initUI( ui )
	...
	self.txt_OmnipotentSoul = TFDirector:getChildByPath(ui,"Label_QimenShengpin_1")
...
function RoleQualityUpLayer.onQualityUpClickHandle( sender )
	...
	if self.isUseOmnipotentSoul > 0 then
		local omnipotentSoul = self.omnipotentSoulList:getObjectAt(1)
		local item_name = omnipotentSoul and omnipotentSoul.name or ""
		CommonManager:showOperateSureLayer(
			...
			nil,
			{
				msg = stringUtils.format(localizable.roleQualityUp_useOmnipotentSoul,item_name.."x"..self.isUseOmnipotentSoul)
			}
		)
		return
    end
...
function RoleQualityUpLayer:onShowLeftInfo()
	...
	CardRoleManager:SetRoleStarTexture(self.cardRole.starlevel, self.star)
...
function RoleQualityUpLayer:onShowRightInfo()
	...
	for k,v in pairs(cost_material) do
		...
		local itemInfo = ItemData:objectByID(k)
		if itemInfo and itemInfo.type == EnumGameItemType.Soul then
			local needOmnipotentSoul ,omnipotentSoulList = BagManager:getOmnipotentSoulNumByQuality( itemInfo.quality )
			if needOmnipotentSoul > 0 then
				self.txt_OmnipotentSoul:setVisible(true)
				local txt = stringUtils.format(localizable.roleStartupPre_omnipotentSoulNum,omnipotentSoulList:getObjectAt(1).name , needOmnipotentSoul)
				self.txt_OmnipotentSoul:setText(txt)
			else
				self.txt_OmnipotentSoul:setVisible(false)
			end

			if curr_num < v then
				if needOmnipotentSoul + curr_num >= v then
					self.isUseOmnipotentSoul = v - curr_num
					self.omnipotentSoulList = omnipotentSoulList
				else
