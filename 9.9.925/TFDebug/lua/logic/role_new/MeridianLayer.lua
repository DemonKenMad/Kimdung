...
function MeridianLayer:initUI(ui)
    ...
    local lbl_lv        = TFDirector:getChildByPath(self.txt_level, 'lbl_lv')
    lbl_lv:setVisible(false);
    ...
    self.bar_percent       = TFDirector:getChildByPath(self.panel_content, 'bar_percent')
    ...
    local verLock = VersionLockData:objectByID(EnumVersionLockType.TrainBreak_Lock)
...
function MeridianLayer:refreshUI()
    ...
    if self.cardRole then
        local verLock = VersionLockData:objectByID(EnumVersionLockType.LianTi_Lock)
        if (verLock == nil or verLock.open == 1) and self.cardRole.quality >= 4 then
        ...
        local redstate = CardRoleManager:checkRoleCanChongXue(self.cardRole)
        CommonManager:setRedPoint(self.btn_level_up, redstate,"isCanCX", ccp(0, 0))
...
function MeridianLayer:registerEvents(ui)
  ...
  --       self.acupointTable[i]['img'].logic = self;
    ...
    self.upLevelResultCallBack = function(event)
        ...
        if self.panel_jingmai.effect then
...
function MeridianLayer.onUplevlToTopClickHandle(sender)
    ...
    if MainPlayer:getVipLevel() < vipLevel then
        ...
        local msg =  stringUtils.format(localizable.vip_yiJianChongXue_not_enough,vipLevel);
...
function MeridianLayer:addXuanzhongEffect( widget , effectName )
    if self.xuanzhong_effect then
        self.xuanzhong_effect:removeFromParentAndCleanup(true)
    end
...
function MeridianLayer:drawRole()
    ...
    self.img_diwen1:setTexture(GetRoleNameBgByQuality(cardRole.quality))
    self.img_quality:setTexture(GetFontByQuality(cardRole.quality))
    local str = "d" .. cardRole.level
    self.txt_level:setText(str)
    self.txt_level:setAnchorPoint(ccp(0.65, 0.5))
    self.txt_power:setText(cardRole:getPowerByFightType(self.fightType))
