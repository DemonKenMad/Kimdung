...
function TrainLayerBreakLayer:loadData(gmId)
	...
	self:addXuanzhongEffect(btn,"meridianeffect")
...
function TrainLayerBreakLayer.onLevelUpClickHandle( sender )
	...
	if nextInfo then
		...
		local goodsType = tonumber(activity[1])
		...
		if bagNum < goodsNum then
            if ResGetWayManager:openResGetLayer(goodsId,goodsType) == true then
                return
            end
