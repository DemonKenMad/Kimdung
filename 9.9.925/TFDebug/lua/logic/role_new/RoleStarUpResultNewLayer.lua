...
function RoleStarUpResultNewLayer:refreshUI()
    ...
    local trainItem_old= RoleTrainData:getRoleTrainByQuality(self.old_quality, self.old_starlevel,self.cardRole.qualityOne)
    local trainItem_new = RoleTrainData:getRoleTrainByQuality(self.cardRole.quality, self.cardRole.starlevel,self.cardRole.qualityOne)
    ...
    CardRoleManager:SetRoleStarTexture(self.old_starlevel, self.img_star)
    --激活显示
    local roleInfoList = RoleTalentData:GetRoleStarInfoByRoleId( self.cardRole.id, self.cardRole.quality)
    ...
    if newFunInfo then
        ...
        CardRoleManager:SetRoleStarTexture(self.cardRole.starlevel, self.newStarArray)       
    end
end
    ...
    for i=1,self.cardRole.maxStar do
        if i == self.cardRole.starlevel then
            ...
            effect:addMEListener(TFARMATURE_COMPLETE,function()
                ...
                CardRoleManager:SetRoleStarTexture(i, {starNode})       
            end)

            effect:setAnimationFps(GameConfig.ANIM_FPS)
            effect:playByIndex(0, -1, -1, 0)
            effect:setPosition(ccp(pos.x+4, pos.y-1))
            effect:setZOrder(1)
            ...
            self.footEffectTimer = TFDirector:addTimer(1000, 1, nil, 
                function()
                    ...
                    effect:setAnimationFps(GameConfig.ANIM_FPS)
                    effect:setPosition(ccp(260,310))
                    effect:setScale(0.8)
                    self.img_role:getParent():addChild(effect,1)
                    self.img_role:setZOrder(2)
                    effect:playByIndex(0, -1, -1, 0)
                    local temp = 0
                    effect:addMEListener(TFARMATURE_UPDATE,function()
                        ...
                        if temp == 13 then
                            ...
                            effect_1:setPosition(ccp(160,50))
                            effect_1:setScale(0.8)
                            self.img_role:getParent():addChild(effect_1,3)
                    ...
                    effect:setPosition(ccp(260,310))
                    effect:setScale(0.8)
                    self.img_role:getParent():addChild(effect,4)        
                    effect:playByIndex(0, -1, -1, 0)
                    end)
