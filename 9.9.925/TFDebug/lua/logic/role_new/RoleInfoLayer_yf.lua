...
function RoleInfoLayer_yf:refreshUI()
    ...
    local offsetY = -180 * fateArray:length()
    if scrollView then
        if self.flag == 0 then
    ...
    panel_hui_yuanfen:setVisible(false)
    panel_liang_yuanfen:setVisible(false)
    self.node_fateList = {}
    self.txt_fateList = {}
    self.txt_titleList = {}
    for i=1,fateArray:length() do
        ...
        local txtMode = TFDirector:getChildByPath(self.node_fateList[i], "txt_yuanfen_word")
        self.txt_fateList[i] = Public:createRichtextHB(txtMode)
        ...
        self.node_fateList[i] :setPosition(ccp(20, (fateArray:length() - i) * 180 + 40))
        local icon = {}
        for k = 1,4 do
            icon[k] = TFDirector:getChildByPath(self.node_fateList[i], "bg_icon"..k)
            icon[k].img_icon = TFDirector:getChildByPath(icon[k], "img_icon")
            icon[k].txt_num = TFDirector:getChildByPath(icon[k], "txt_num")
            icon[k]:setVisible(false)
        end
        self.node_fateList[i].icon = icon
    ...
    for i=1,fateArray:length() do
        ...
        local txtMode = TFDirector:getChildByPath(self.node_fateLList[i], "txt_yuanfen_word")
        self.txt_fateLList[i] = Public:createRichtextHB(txtMode)
        ...
        self.node_fateLList[i]:setPosition(ccp(20, (fateArray:length() - i) * 180 + 40))
        local icon = {}
        for k = 1,4 do
            icon[k] = TFDirector:getChildByPath(self.node_fateLList[i], "bg_icon"..k)
            icon[k].img_icon = TFDirector:getChildByPath(icon[k], "img_icon")
            icon[k].txt_num = TFDirector:getChildByPath(icon[k], "txt_num")
            icon[k]:setVisible(false)
        end
        self.node_fateLList[i].icon = icon
    ...
    for fate in fateArray:iterator() do
        ...
        elseif FengYunLuManager and FengYunLuManager.isOpenFengYunLu  == true then
            local f = fate:gettarget()
            ...
            local isUpBattle = FengYunLuManager:getisUpByroleid(self.cardRole.id)
            if isUpBattle == true then
                for v,k in pairs(f) do
                    if k.fateType == 1 then
                        local roleid = FYRecordData:getidByRoleId(k.fateId)
                        if roleid then
                            for i = 1,9 do
                                if FengYunLuManager.MyRoleList[i] == roleid then
                                    self.node_fateLList[index]:setVisible(true);  
                                    self.node_fateList[index]:setVisible(false);
                                end              
                            end
        ...
        self.txt_fateList[index]:setText(stringUtils.richText(fate.details));
        self.txt_fateLList[index]:setText(stringUtils.richText(fate.details));
        self.txt_titleList[index]:setText(fate.title);
        self.txt_titleLList[index]:setText(fate.title);
        self:Updatafateicon(self.node_fateList[index].icon,fate.target)
        self:Updatafateicon(self.node_fateLList[index].icon,fate.target)

        index = index +1;
    end
    scrollView:setInnerContainerSizeForHeight(180 * fateArray:length() + 40)
...
function RoleInfoLayer_yf:Updatafateicon(iconlist,target)
    for i = 1, 4 do
        iconlist[i]:setVisible(false)
    end
    local fatelist  = string.split(target,'|')
    for k,v in pairs(fatelist) do
        local temp = string.split(v,',')  
        iconlist[k]:setVisible(true) 
        local info 
        if (tonumber(temp[1])) == 1 then
            info = RoleData:objectByID(tonumber(temp[2]))
            iconlist[k]:setTexture(GetColorIconByQuality(info.quality))
            iconlist[k].img_icon:setTexture(info:getIconPath())
            -- iconlist[k].img_icon:setScale(0.9)
        elseif (tonumber(temp[1])) == 2 then
            info = ItemData:objectByID(tonumber(temp[2]))
            iconlist[k]:setTexture(GetColorIconByQuality(info.quality))
            iconlist[k].img_icon:setTexture(info:GetPath())
            -- iconlist[k].img_icon:setScale(1)
        end
    end
...
function RoleInfoLayer_yf:canUseItemActivateFate( fate )
    ...
    for v in itemList:iterator() do
        if (quality >= CardRoleManager.MaxQuality or v.quality >= quality) and v.usable >= num+1 then
