...
function qimenduntupoLayer:cellInfoSet(cell, panel, idx)
    ...
    for i=panelIndex-1,panelIndex do
        cell.infoPanel[i].titleName:setText(stringUtils.format(localizable.qimenduntupo_text1, idx ))
    ...
    if self.effectLevel and self.effectLevel == idx then
        ...
        self:showRefreshEffect(cell,panel,attrNum,true)
    else
        self:showRefreshEffect(cell,panel,attrNum,false)
    end
...
function qimenduntupoLayer.onTopuBtnClickHandle(btn)
    ...
    local maxOpenLevel = ConstantData:objectByID("Player.Level.Max").value
    if info then
        if info.level > MainPlayer:getLevel() then
            ...
            if info.level > maxOpenLevel then
                toastMessage(localizable.qimenduntupo_no_open)
                return
            end
...
function qimenduntupoLayer:showRefreshEffect(cell,panel,attrNum,isshow)
    if self.refreshEffect and self.refreshEffect:getParent() == panel then
        self.refreshEffect:removeMEListener(TFARMATURE_COMPLETE) 
        self.refreshEffect:removeFromParent()
        self.refreshEffect = nil
    ...
    TFResourceHelper:instance():addArmatureFromJsonFile(filePath)
    effect = TFArmature:create("cellRefresh_anim")
    effect:setAnimationFps(GameConfig.ANIM_FPS)
    effect:playByIndex(0, -1, -1, 0)
    effect:setVisible(true)
    ...
    panel:addChild(effect,100,9999)
