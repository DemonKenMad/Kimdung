...
function BagLayer:initUI(ui)
    ...
    self.tab = {
        TFDirector:getChildByPath(ui, 'btn_all'),
        TFDirector:getChildByPath(ui, 'btn_prop'),
        TFDirector:getChildByPath(ui, 'btn_wx'),
        TFDirector:getChildByPath(ui, 'btn_tianshu'),
        TFDirector:getChildByPath(ui, 'btn_xinfa'), 
        TFDirector:getChildByPath(ui, 'btn_zhenfa'),
        TFDirector:getChildByPath(ui, 'btn_soul'),
        TFDirector:getChildByPath(ui, 'btn_piece')
    }
    ...
    self.normalTextures = {
        'ui_new/bag/tab-dj2.png',
        'ui_new/bag/tab-wx2.png',
        'ui_new/tianshu/tab_ts.png',
        'ui_new/bag/tab_xf.png',
        'ui_new/bag/tab_zf.png', 
        'ui_new/bag/tab-xh2.png',       
        'ui_new/bag/tab_my.png',
        'ui_new/bag/tab-sp2.png'
    }
    self.selectedTextures = {
        'ui_new/bag/tab-dj.png',
        'ui_new/bag/tab-wx.png',     
        'ui_new/tianshu/tab_tsh.png',
        'ui_new/bag/tab_xfh.png',
        'ui_new/bag/tab_zfh.png',
        'ui_new/bag/tab-xh.png',
        'ui_new/bag/tab_my2.png',
        'ui_new/bag/tab-sp.png'
    }
   ...
   --  self.selectedTextures = {'ui_new/bag/tab-qb.png','ui_new/bag/tab-dj.png','ui_new/bag/tab-wx.png','ui_new/bag/tab-xh.png','ui_new/bag/tab-sp.png'}
    self.layerPath = {
        'lua.logic.bag.ItemDetails',
        'lua.logic.bag.ItemDetails', 
        'lua.logic.bag.SkyBookDetailsLayer',
        'lua.logic.bag.XinFaDetailsLayer',
        'lua.logic.bag.ZhenFaDetailsLayer', 
        'lua.logic.bag.BagSoulDetailsLayer',       
        'lua.logic.bag.MiYaoDetails',
        'lua.logic.bag.BagPieceDetailsLayer'
    }
    self.detailsLayer = {}
    ...
    BagManager:setBagRedPoint(false)
...
function BagLayer:select(holdGoods)
    ...
    local function calculateIndex(holdGoods)
        ...
        if _type == EnumGameItemType.Item or _type == EnumGameItemType.Box or _type == EnumGameItemType.RandomPack or _type == EnumGameItemType.HeadPicFrame or _type == EnumGameItemType.ItemPiece then
            return EnumBagItemType.DaoJu
        elseif _type == EnumGameItemType.Book or (_type == EnumGameItemType.Piece and _kind == 10) then
            return EnumBagItemType.WuXue
        elseif _type == EnumGameItemType.Soul then
            return EnumBagItemType.XiaHun
        -- elseif _type == EnumGameItemType.Piece and _kind == 3 then
        --[[
            changed by wuqi
        ]]
        elseif _type == EnumGameItemType.SkyBook then
            return EnumBagItemType.TianShu
        elseif _type == EnumGameItemType.StrategyCard or _type == EnumGameItemType.StrategyPiece then
            return EnumBagItemType.ZhenFa

        elseif _type == EnumGameItemType.MiYao then
            return EnumBagItemType.MiYao
        elseif _type == EnumGameItemType.Piece and ( _kind >= 1 and _kind <= 5) then
            return EnumBagItemType.WQSuiPian
        elseif _type == EnumGameItemType.XinFa or  _type == EnumGameItemType.XinFaPiece then 
            return EnumBagItemType.XinFa
        --[[
        elseif _type == EnumGameItemType.Piece and ( _kind >= 1 and _kind <= 5) then
            return 4
        ...
        elseif _type == EnumGameItemType.Piece and ( _kind == 25 ) then --秘要碎片
            return EnumBagItemType.WQSuiPian
        else
            return EnumBagItemType.DaoJu
        end
    ...
    if holdGoods.itemdata and index ~= EnumBagItemType.MiYao then
        self.goodsId = holdGoods.itemdata.id
    elseif index == EnumBagItemType.ZhenFa then
        self.goodsId = holdGoods.gmId
    elseif index == EnumBagItemType.MiYao then
    ...
    self.bagTableView:select(self.goodsId,holdGoods.type)
end
...
function BagLayer:refreshRedPointState()
    ...
    local bLimit = BagManager:isHaveTimeLimitItem()
    if bLimit then
        CommonManager:updateRedPoint(self.tab[1], bLimit,ccp(0,-20))
        CommonManager:setRedPointMode(self.tab[1],2,ccp(0,-20))
    else
        CommonManager:setRedPointMode(self.tab[1],1,ccp(0,-20))
    end

    local canZhaomu = BagManager:isHaveCanZhaomu()
    CommonManager:updateRedPoint(self.tab[3],canZhaomu,position)
    local canMerge = BagManager:isHaveCanMiYaoPiece()
    --CommonManager:updateRedPoint(self.tab[6],canMerge,position)

    local canMerge = BagManager:isHaveCanEquipPiece()
    CommonManager:updateRedPoint(self.tab[7],canMerge,position)

    -- local canStrategyCard = BagManager:isHaveCanStrategyCardPiece()
    -- CommonManager:updateRedPoint(self.tab[5],canStrategyCard,position)
