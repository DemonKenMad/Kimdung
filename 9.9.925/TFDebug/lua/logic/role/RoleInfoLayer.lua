...
function RoleInfoLayer:loadSelfData(selectIndex,roleList)
    self.selectIndex    = selectIndex;
    self.cardRoleId     = nil;
    self.roleList       = roleList or CardRoleManager.cardRoleList; -- add by king 
    self.type           = "self";
end
...
function RoleInfoLayer.onSkillItemClickHandle(sender)
    ...
    if sender.level ~= -1 then
        if self.cardRole:getIsMainPlayer() then
           ...
           CardRoleManager:openSkillInfo(self.cardRole.id, spellInfo,spellLevelInfo,nil,self.cardRole.XinFaSpecialAttribute )
        else
