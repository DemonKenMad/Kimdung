...
function EquipuninstallLayer:initAttr(pannl_node,equip)
    ...
    for i=1,(EnumAttributeType.Max-1) do
        if baseAttr[i] then
            ...
            txt_attr_base_val:setPositionX(99)
        end
    end
    ...
    Public:showAttributeMove( self.panelMove, equip )   
...
function EquipuninstallLayer.onImproveClickHandle(sender)
     ...
     if self.equipGmId then
        if equipItem.equip ~= 0 then
            local role = CardRoleManager:getRoleById(equipItem.equip)
            if role and role.pos and role.pos > 0 then
                local equipList = ZhengbaManager:getOnlyStrategyEquip(self.logic.roleList,EnumFightStrategyType.StrategyType_PVE)
                EquipmentManager:openSmithyLayer(self.equipGmId, equipList, 0,false) 
            else
                EquipmentManager:openSmithyLayer(self.equipGmId, nil, nil,false) 
            end
        else
            EquipmentManager:openSmithyLayer(self.equipGmId,nil,equipItem.equipType,true)
        end
     else
