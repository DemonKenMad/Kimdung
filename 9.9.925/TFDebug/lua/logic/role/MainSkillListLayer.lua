...
function MainSkillListLayer:ctor(isFourSkill)
...
function MainSkillListLayer:initUI(ui)
    ...
    self.btn_tap4 = TFDirector:getChildByPath(ui, 'btn_tap4')
    if self.skillNumber == 4 then
        self.btn_tap4:setVisible(true)
        self.groupButtonManager = GroupButtonManager:new( { [1] = self.btn_tap1, [2] = self.btn_tap2, [3] = self.btn_tap3, [4] = self.btn_tap4});
    else
        self.btn_tap4:setVisible(false)
...
function MainSkillListLayer.onUpLevelClickHandle(sender)
    ...
    if MainPlayer:isEnoughTimes(EnumRecoverableResType.SKILL_POINT, 1, true) then
        local skillMaxLevel = MainPlayer:getRoleSkillMax()
...
function MainSkillListLayer.onSkillItemClickHandle(sender)
    ...
    if sender.level ~= -1 then
        if self.cardRole:getIsMainPlayer() then
            local spellInfo = sender.spellInfo;
            -- <<<<<<<<<<<<<<<<<<<技能替换判断
            local replaceSkillId = nil
            if spellInfo.hidden_skill == 3 then
                 replaceSkillId = CardRoleManager:isSkillReplace(self.cardRole.id, self.cardRole.starlevel, spellInfo.id ,self.cardRole.skillStarLevel, self.cardRole.quality)                 
            else
                 replaceSkillId = CardRoleManager:isSkillReplace(self.cardRole.id, self.cardRole.starlevel, spellInfo.id, nil, self.cardRole.quality)   
            end
            ...
            local spellInfo = sender.spellInfo;
            -- <<<<<<<<<<<<<<<<<<<技能替换判断
            local replaceSkillId = CardRoleManager:isSkillReplace(self.cardRole.id, self.cardRole.starlevel, spellInfo.id, nil, self.cardRole.quality) 
            if replaceSkillId ~= spellInfo.id then
                spellInfo = SkillBaseData:objectByID(replaceSkillId)
            end
            -- >>>>>>>>>>>>>>>>>>>>
...
function MainSkillListLayer:showFourSkillDetail()
    ...
    local spellInfoConfig = self.cardRole.leadingSpellInfoConfigList:objectAt(starIndex);
    self.node_skillArr[1]:setVisible(false);
    self.node_skillArr[2]:setVisible(false);
    self.node_skillArr[3]:setVisible(false);
    if spellInfoConfig then
        ...
        if spellInfo then
            ...
            -- <<<<<<<<<<<<<<<<<<<技能替换判断
            local replaceSkillId = CardRoleManager:isSkillReplace(self.cardRole.id, self.cardRole.starlevel, spellInfo.id,self.cardRole.skillStarLevel, self.cardRole.quality) 
            if replaceSkillId ~= spellInfo.id then
        ...
        -- >>>>>>>>>>>>>>>>>>>>
            ...
            if self.cardRole.quality < 6 then
                ...
                self.btn_skillArr[index].des = localizable.RoleInfoLayer_py_red
                ...
                self.txt_openlevelArr[index]:setText(localizable.RoleInfoLayer_py_red);
            else
                self.img_openqualityArr[index]:setVisible(false);
...
function MainSkillListLayer:refreshRedPoint()
    local skillMaxLevel = ConstantData:objectByID("RoleSkill.Max.Level").value or 150

    for i,spell in pairs(self.cardRole.leadingRoleSpellList) do
        if spell.choice then
            if spell.spellId.level < self.cardRole.level and spell.spellId.level < skillMaxLevel then
                local levelInfo = SkillLevelData:objectByID(spell.spellId);
                local index = math.ceil(i/3)
                local btn = self.groupButtonManager.btnDic[index]
                if MainPlayer:isEnoughCoin(levelInfo.uplevel_cost, false) and MainPlayer:isEnoughTimes(EnumRecoverableResType.SKILL_POINT,10,false,false) then
                    CommonManager:setRedPoint(btn, true,"isSkillPointFull",ccp(0,0))
                else
                    CommonManager:setRedPoint(btn, false,"isSkillPointFull",ccp(0,0))
                end
            end
        end
    end
        ...
        if spellInfoConfig then
            ...
            if spellInfo then
                ...
                elseif not spellInfoConfig:GetLevelIsOpen(self.cardRole.level) then
                    ...
                    if self.cardRole.quality < 6 and selectIndex == 4 then
                        self.img_skillArr[index]:setColor(ccc3(166, 166, 166));
                        -- self.btn_skillArr[index].des = "主角" .. spellInfoConfig.enable_level .. "级可用"
                        self.btn_skillArr[index].des = localizable.RoleInfoLayer_py_red
                        self.bgArr[index].des = self.btn_skillArr[index].des
...
function MainSkillListLayer:registerEvents(ui)
    ...
    self.btn_tap4.logic = self;
    self.btn_tap4:addMEListener(TFWIDGET_CLICK, audioClickfun(self.onTypeSelectClickHandle))
    ...
    self.LevelUpCallBack = function(event)
        ...
        function showToastMessage()
            ...
            oldId.skillId = CardRoleManager:isSkillReplace(self.cardRole.id, self.cardRole.starlevel, oldId.skillId, nil, self.cardRole.quality) 
            newId.skillId = CardRoleManager:isSkillReplace(self.cardRole.id, self.cardRole.starlevel, newId.skillId, nil, self.cardRole.quality) 
            ...
            tempAddArr[2] = 0
            -- newLevel.attr_add - oldLevel.attr_add
            ...
            tempAddArr[12] = 0
            -- newLevel.buff_id - oldLevel.buff_id
