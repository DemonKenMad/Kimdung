...
RoleEquipInfoLayer.LIST_ITEM_HEIGHT = 150; 
...
function RoleEquipInfoLayer.cellSizeForTable(tableView,idx)
    return RoleEquipInfoLayer.LIST_ITEM_HEIGHT, 960
end
...
---------   换地方 by stephen
    ...
    if (curEquip ~= nil and  curEquip.gmId == equipItem.gmId) then
        ...
        if role.isMainPlayer then
            ...
            txt_equiped_name:setText(MainPlayer.name)
        else
            txt_equiped_name:setText(role.name)
        end
    end
...
function RoleEquipInfoLayer.onCellClickHandle(sender) 
    ...
    if self.selectEquipItem and self.selectEquipItem.img_xuanzhong and not tolua.isnull(self.selectEquipItem.img_xuanzhong) then
...
function RoleEquipInfoLayer:openEquipDetail(equipItem) 
    ...
    if not tolua.isnull(self.curLeyer) and self.curLeyer and self.curLeyer.getParent then
        self.curLeyer:getParent():removeLayer(self.curLeyer, not self.curLeyer.isCache);
    end
    local curEquip = nil;
    if self.type then
        local equipment = self.cardRole:getEquipment()
        if (equipment) then
            curEquip = equipment:GetEquipByType(self.type);
        end
    end
