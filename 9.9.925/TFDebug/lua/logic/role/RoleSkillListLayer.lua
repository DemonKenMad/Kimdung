...
function RoleSkillListLayer:ctor(isFourSkill)
    self.super.ctor(self, data)
    self.skillNumber = 3
    if isFourSkill then
        self.skillNumber = 4
        self:init("lua.uiconfig_mango_new.role.RoleSkillListLayer2")
    else
...
function RoleSkillListLayer:refreshUI()
    ...
    -- 测试代码
    -- self.cardRole.spellLevelIdList[4] = {skillld = 31504,level = 10}

    for index = 1, self.skillNumber do
        ...
        if spellInfo then
            ...
            if self.cardRole.martialLevel < openQuality[index] then
                ...
                -- 侠客拥有四个技能不能使用 但还是需要显示
                if index == 4 and self.cardRole.quality < 6 then
                    self.img_skillArr[index]:setColor(ccc3(166, 166, 166));
                    -- self.btn_skillArr[index].des = QUALITY_STR[openQuality[index]] .. "品解锁"
                    -- self.btn_skillArr[index].des = "武学"..EnumWuxueLevelType[openQuality[index]] .. "重解锁"
...
function RoleSkillListLayer:registerEvents(ui)
    ...
    self.LevelUpCallBack = function(event)
        ...
        function showToastMessage()
            ...
            if newLevel.buff_id ~= 0 and newLevel.type ~= EnumFightSkillType.type_13 then
        ...
        self.levelupTimeId = TFDirector:addTimer(200, 1, nil, function()
            -- self:refreshUI();
            if self.txt_levelArr and self.txt_levelArr[selectIndex] then      
                local toastTween = {
