...
function RoleTransferLayer:initUI(ui)
    ...
    self.btn_transfer2       = TFDirector:getChildByPath(ui, 'btn_transfer2')
...
function RoleTransferLayer:refreshUI()
    ...
    self.selectIndex = self.roleMainList:indexOf(self.cardRole.id)
...
function RoleTransferLayer:setRoleExpInfo()
    ...
    CardRoleManager:SetRoleStarTexture(self.cardRole.starlevel, self.img_star)
...
function RoleTransferLayer:registerEvents()
    ...
    self.btn_transfer2:addMEListener(TFWIDGET_CLICK, audioClickfun(self.OneKeyTransferClickHandle),1)
...
function RoleTransferLayer.TransferClickHandle(sender)
    ...
    for v in self.catfood:iterator() do
        ...
        if item.kind ~= 3 then
            if item.quality >= QUALITY_JIA then
                 isHaveQualityJia = true
            end
        end
    end
...
function RoleTransferLayer.tableCellAtIndex(table, idx)
    ...
    if nil == cell then
        ...
        for i=1,numInCell do
            ...
            equip_panel:setPosition(ccp(140*(i-1),0))
...
function RoleTransferLayer:addPage(pageIndex) 
    ...
    local roleId = self.roleMainList:objectAt(pageIndex)
...
function RoleTransferLayer:drawRole()
    local roleId    = self.roleMainList:objectAt(self.selectIndex);
    ...
    self.roleList = CardRoleManager:getOtherNotUsed(self.roleGmId)
    self.roleList:sort(cmpFun)
    self.soulList = BagManager:getItemByTypeWithKind(EnumGameItemType.Soul,{1,3})
    -- for v in self.soulList:iterator() do
    --     if v.type == EnumGameItemType.Soul and v.kind == 2 then
    --         self.soulList:removeObject(v)
    --     end
...
function RoleTransferLayer.OneKeyTransferClickHandle(sender)
    local self = sender.logic
    self:onKeyToMaxlevel()
end


function RoleTransferLayer:onKeyToMaxlevel()
    if self.cardRole == nil then
        return
    end
    local maxLevel = MainPlayer:getLevel()
    local maxExp = LevelData:getMaxRoleExp(maxLevel)
    -- if self.cardRole.level > maxLevel then
    --     return
    -- elseif self.cardRole.level == maxLevel and self.cardRole.curExp >= maxExp then
    --     return
    -- end

    if self.soulList:length() == 0 then
        toastMessage(localizable.OneKeyToMaxLevel_Text1)
        return
    end
   
    if self.cardRole.curExp >= maxExp then
        toastMessage(localizable.OneKeyToMaxLevel_Text2)
        return
    end
    local maxCoin = MainPlayer:getCoin()
    -- if maxCoin < TransferSoulCost then
    --     toastMessage(localizable.common_your_yuanbao)
    --     return
    -- end
    local needExp = LevelData:getMaxRoleExp(self.cardRole.level) - self.cardRole.curExp
    for i=self.cardRole.level+1 , maxLevel do
        needExp = needExp + LevelData:getMaxRoleExp(i)
    end
    -- needExp = math.min(needExp,maxCoin*10)
    local catfoodlist = {}
    for v in self.soulList:iterator() do
        if v.kind == 3 then
            local per_exp = v.itemdata.provide_exp
            local num = math.min(math.ceil(needExp/per_exp) , v.num )
            num = math.min(num , math.floor(maxCoin*10/per_exp))
            if num > 0 then
                needExp = needExp - num*per_exp
                maxCoin = maxCoin - num*per_exp*0.1
                local tbl = {
                    v.id,
                    num,
                }
                catfoodlist[#catfoodlist+1] = tbl
                if needExp <= 0 then
                    break
                end
            else
    ...
    CardRoleManager:roleTransfer( self.cardRole.gmId , {} , catfoodlist)
end
