...
function FightMainLayer:showPage(pageIndex)
    ...
    if self.currStateType == MultiServerFightManager.ActivityState_1 then
        ...
        if (self.rankDataInfo.ranks ~= nil) then
...
function FightMainLayer.onBuyQualificationClick( btn )
    ...
    local num = BagManager:getItemNumById(30121) + BagManager:getItemNumById(30140)
    if num <= 0 then
        if MallManager:checkShopOneKey(30140) == true then
            return
        end
        if MallManager:checkShopOneKey(30121) == true then
            return
        end
...
function FightMainLayer:addBgEffect()
