...
function IllustrationRoleLayer:initUI(ui)
    ...
    self.QualityRes = {}
    self.QualityRes[6] = "ui_new/handbook/xiake_6.png"
...
function IllustrationRoleLayer:drawRole(node, quality, index)
    ...
    -- end
    local bUp = CardRoleManager:getShengPingByCard( role )
    img_shengpinicon:setVisible(bUp)
    ...
    btn_head.bMaxQuality = false
    local quality = 0
    if (bUp and role.quality == 6) then
        quality = role.quality + 1 
        btn_head.bMaxQuality = bUp
    end
    CardRoleManager:BindEffectOnEquip(img_touxiang, quality);   
...
function IllustrationRoleLayer.touchRoleNode(sender)
   ...
   IllustrationManager.isOpenTuJian = true
   AlertManager:show()
end
