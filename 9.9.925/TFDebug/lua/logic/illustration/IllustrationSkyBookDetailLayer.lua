...
function IllustrationSkyBookDetailLayer:initUI(ui)
    ...
    for i=1,5 do
        ...
        self.attr_list_panel[i].txt = TFDirector:getChildByPath(ui, 'txt_quality'..i)
        self.attr_list_panel[i].num = TFDirector:getChildByPath(self.attr_list_panel[i].txt, 'txt_num1')
    end
...
function IllustrationSkyBookDetailLayer:draw()
    ...
    self.txt_basePowerNum:setText(self:getAttrName(skyBook.config.usable))
...
function IllustrationSkyBookDetailLayer:getAttrName( kind )
    local tKind = {}
    while(kind > 0) do
