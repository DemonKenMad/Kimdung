...
function QiYuanLayer:OnGetDayRewardClick( sender )
    ...
    if rewardDay < maxDay then
        ...
        if self.rewardList == nil then
            ...
            for v in InvocatoryRewardData:iterator() do
                if v.index_id == 4 then
                    local rewardInfo = {}
                    rewardInfo.type = EnumDropType.ROLE
                    rewardInfo.itemId = v.role_id
                    rewardInfo.number = 1
        ...
        layer:setRewardList(self.rewardList,localizable.zadan_tipsRoles)
        AlertManager:show()
        return
    end
...
function QiYuanLayer.sortFunc(cardRoleId1, cardRoleId2)
...
function QiYuanLayer:getRoleList()
    ...
    for roleId in CardRoleManager.cardRoleList:iterator() do
        ...
        if card.quality <= 7 and card:getIsMainPlayer() == false then
            self.role_list:pushBack(roleId)
        end
    end
...
function QiYuanLayer:roleSelected()
    ...
    CardRoleManager:SetRoleStarTexture(cardRole.starlevel, self.img_starList)
end
