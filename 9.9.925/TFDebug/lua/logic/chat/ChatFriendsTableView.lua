...
function ChatFriendsTableView:selectCell(selectedPlayerID)
    ...
    local preSelectedPlayerID = localVars.selectedPlayerID
    local preSelectedCellIndex = nil
    ...
    -- 获取cell的index
    for i, v in ipairs(localVars.friendList) do
        if v.playerId == selectedPlayerID then
            cellIndex = i
        end

        if v.playerId == preSelectedPlayerID then
            preSelectedCellIndex = i
        end
    end
    ...
    while cell ~= nil do
        cell.friendCell:unselected()
        index = index + 1
        cell = self.tableView:cellAtIndex(index)
    end

    if preSelectedCellIndex and self.tableView:cellAtIndex(preSelectedCellIndex - 1) then
        self.tableView:cellAtIndex(preSelectedCellIndex - 1).friendCell:unselected()
    end
    ...
    if self.tableView:cellAtIndex(cellIndex - 1) then
        self.tableView:cellAtIndex(cellIndex - 1).friendCell:selected()
    end
    -- 选中
