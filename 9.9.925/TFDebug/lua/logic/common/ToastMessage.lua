...
function toastMessage(text,position,size,showType,isAdd, nTime)
    ...
    isAdd = ( (nCount < nMaxCount) and nCount > 0)
   ...
   bgImg:setSize(ccs(math.max(math.min(bgImg:getSize().width * string.utf8len(text)/20, bgImg:getSize().width),400)  ,bgImg:getSize().height)) 
    ...
    toastMessageLayer:beginToast(isAdd, nTime);
    nCount = nCount + 1
...
function ToastMessage:beginToast(isAdd, nTime)
    local currentScene = Public:currentScene();
    self:setZOrder(500);
