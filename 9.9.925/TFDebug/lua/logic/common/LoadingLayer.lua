...
function LoadingLayer:setType(showType)
    ...
    if showType == 1 then
        ...
        skillEff = TFArmature:create("loading_simple_anim")
    ...
    if showType == 2 then
        ...
        TFResourceHelper:instance():addArmatureFromJsonFile(resPath)
        skillEff = TFArmature:create("loading_anim")
        -- skillEff:setScaleX(1.05)
        -- skillEff:setScale(0.4)
        skillEff:setPosition(ccp(0,320))
...
function LoadingLayer:hide()
    ...
    if LoadingLayer.loadingCount == 0 then
        ...
        local loading = LoadingLayer.loading;
        if loading then
            if (loading.toScene and  not tolua.isnull(loading.toScene)) then
