...
function AppointLayer:initUI( ui )
    ...
    local idLeader = FactionManager:getPostInFaction()
    if idLeader == FactionManager.DeputyLeader then
        self.btn_sr:setGrayEnabled(true)
        self.btn_member:setGrayEnabled(true)
        self.btn_DeputyLeader:setGrayEnabled(true)
        --
        self.btn_sr:setTouchEnabled(false)
        self.btn_member:setTouchEnabled(false)
        self.btn_DeputyLeader:setTouchEnabled(false)
    end
end
...
function AppointLayer.deleteButtonClick( btn )
    local self = btn.logic
    local idLeader = FactionManager:getPostInFaction()
    if idLeader ~= FactionManager.Leader and idLeader ~= FactionManager.DeputyLeader then
        toastMessage(localizable.common_no_power)
        return
    end
