...
function FactinoMembers:showStarHeroEffect( cell,isStarHero,vipLevel )
    if StarHeroManager:getServerSwitchStatue() == true and isStarHero and isStarHero > 0 then
    	if cell.starHeroEffect ~= nil then
    		cell.starHeroEffect:removeFromParent()
    		cell.starHeroEffect = nil
    	end
    	local bgSize = cell.headBtn:getSize()
        cell.starHeroEffect = Public:getStarHeroEffect()
        cell.starHeroEffect:playByIndex(0,-1,-1,1)
       	cell.starHeroEffect:setScale(0.85)
        cell.starHeroEffect:setVisible(true)
        local posY = cell.vip:getPositionY() + bgSize.height * 0.5 - 3
        cell.starHeroEffect:setPositionY(posY)
        cell.headBtn:addChild(cell.starHeroEffect,100)
        
        cell.starHeroEffect:setVisible(true)
        if vipLevel == nil or SettingManager.TAG_VIP_YINCANG == tonumber(vipLevel) then
            cell.starHeroEffect:setPositionX(cell.vip:getPositionX() + bgSize.width * 0.5 + 45)
        else
            cell.starHeroEffect:setPositionX(cell.vip:getPositionX() + bgSize.width * 0.5 + 145)
        end
    else
        if cell.starHeroEffect then
            cell.starHeroEffect:setVisible(false)
        end
    end
end
...
function FactinoMembers:cellInfoSet( cell, panel, idx )
	if cell.boundData == nil then
		...
		cell.btnCancelImpeach:setTouchEnabled(false)
	...
	Public:addInfoListen(cell.headBtn,true,3,cellItem.playerId)
	if cellItem.playerId == MainPlayer:getPlayerId() then
		--只显示结交奖励按钮
		Public:addInfoListen(cell.headBtn,false)
	...
	elseif cellItem.competence == FactionManager.Leader then
		...
		if self.currState == 3 then
			...
			local currTime = math.floor(self.operateTime/1000) - MainPlayer:getNowtime()
			local timeStr = FactionManager:getTimeString( currTime )
			print("弹劾中",timeStr)
	...
	if(self.myPost == FactionManager.DeputyLeader) then
		if cellItem.competence ~= FactionManager.Leader and cellItem.competence ~= FactionManager.DeputyLeader then
			cell.btnAppoint:setVisible(true)
		end
	end
	--结交按钮处理
	...
	if RoleIcon == nil then
		--print("cellItem.playerId == ",cellItem.playerId)
    ...
    self:showStarHeroEffect(cell,cellItem.starHeroRanking,cellItem.vip)
...
function FactinoMembers.impeachButtonClick( btn )
	...
	elseif FactionManager:isCanImpeach() then
	    ...
	    FactionManager:openTanHeLayer()
		
	else
		--toastMessage("发起人必须等级大于等于30级才可以发起弹劾")
		toastMessage(localizable.factionMembers_tanhe_level)
	end
end
