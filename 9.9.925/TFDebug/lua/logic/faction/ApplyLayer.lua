...
function ApplyLayer:cellInfoSet(cell, panel, idx)
	...
	if cell.info.isMoreButton == false and item then
...
function ApplyLayer.createButtonClick(btn)
	...
	local openLevel = FunctionOpenConfigure:getOpenLevel(1205)
	local currLevel = MainPlayer:getLevel()
	if currLevel < openLevel then
		toastMessage(localizable.factionApply_open)
		return
	end
