...
AssistFightManager.QiheMaxLevelSwitch = true --false：最高5级 true:最高8级
--t_s_assistant_rule
...
function AssistFightManager:openAssistWithType(Type)
	...
	local roleNumber = ZhengbaManager:getFightRoleNum( Type ) or 0
	if roleNumber <= 0 then
		toastMessage(localizable.AssistFightManager_NoRole)
		return
	end
...
function AssistFightManager:getCardRoleList( Type )
	...
	elseif Type == LineUpType.LineUp_BloodyBattle then
		for roleId in CardRoleManager.cardRoleList:iterator() do
			local cardRole = CardRoleManager:getRoleById(roleId)
			if cardRole.level>=10 and  cardRole.blood_pos and cardRole.blood_pos ~= 0 then
			-- elseif needAdd( cardRole.gmId, self.roleInfo[Type], nil) then
			elseif cardRole.level>=10 then 
	...
	elseif Type == LineUpType.LineUp_Mine1_Defense then
		...
		for roleId in normalRoleList:iterator() do
			local cardRole = CardRoleManager:getRoleById(roleId)
			-- if needAdd(cardRole.gmId, self.roleInfo[Type], defenseList) then
			if 1 then
				local card_copy = clone(cardRole)
	...
	elseif Type == LineUpType.LineUp_CHUANGZHEN then
		local normalRoleList =  ZhengbaManager:getRoleList(EnumFightStrategyType.StrategyType_CHUANGZHEN)	
		for roleId in normalRoleList:iterator() do
			local cardRole = CardRoleManager:getRoleById(roleId)
			-- if needAdd(cardRole.gmId, self.roleInfo[Type], defenseList) then
			if 1 then
	...
	elseif Type == LineUpType.LineUp_CROSSZHENGBA then
		local normalRoleList =  ZhengbaManager:getRoleList(EnumFightStrategyType.StrategyType_CROSSZHENGBA)	
		for roleId in normalRoleList:iterator() do
			local cardRole = CardRoleManager:getRoleById(roleId)
			-- if needAdd(cardRole.gmId, self.roleInfo[Type], defenseList) then
			if 1 then
...
-- for test cardRoleList--
	...
	elseif Type == LineUpType.LineUp_CHUANGZHEN then
		local defenseList = ZhengbaManager:getFightList( EnumFightStrategyType.StrategyType_CHUANGZHEN )		
		for cardRole in cardRoleList:iterator() do
			if needAdd(cardRole.gmId, self.roleInfo[Type], defenseList) then
				cardRoleFateList:push(cardRole)
			end
	...
	elseif Type == LineUpType.LineUp_CROSSZHENGBA then
		local defenseList = ZhengbaManager:getFightList( EnumFightStrategyType.StrategyType_CROSSZHENGBA )		
		for cardRole in cardRoleList:iterator() do
			if needAdd(cardRole.gmId, self.roleInfo[Type], defenseList) then
				cardRoleFateList:push(cardRole)
			end
...
function AssistFightManager:getStrategyPower( Type ,isOnlySelfPower)
	local strategyList = self:getStrategyList( Type )
	...
	for i=1,10 do
		if strategyList[i] and strategyList[i] ~= 0 then
			...
			if (isOnlySelfPower and CardRoleManager.isOnlySelfPower) then
				if role then
					allPower = allPower + role:getNotTeamPowerByIdList(roleIdList,Type)
				end
			else
				if role then
...
function AssistFightManager:getQihePreviewInfo(Type)
	...
	if self.roleInfo[Type] then
		...
		for k,v in pairs(self.roleInfo[Type]) do
			if v > 0 then
				...
				if cardRole then
					...
					for index, attr in pairs(attrTble) do
						-- print('attrTble = ',attrTble)
						for _,attrIndex in pairs(attr) do
							local attridx = tonumber(attrIndex)
							-- print('attridx = ',attridx)
							local num = cardRole:getTotalAttributeWithOutQihe(attridx)
							num = math.floor((percent[index]/10000)*num)
		...
		local achieveAttr,subAchieve = self:getAchieveTotalAttr(achieveLevel)
		subAchieveAttr = subAchieve
...
function AssistFightManager:getAchieveTotalAttr(level)
	...
	for v in AgreeConditionAttributeData:iterator() do
		if v.id <= level then
			...
			elseif v.effect_active ~= "" or v.effect_passive ~= "" then
				local tbl = {}
				tbl.des = v.des
				tbl.power = v.power
				table.insert(subTbl, tbl)
			end
		else
			return attrTbl,subTbl
		end
	end
	return attrTbl,subTbl
end

function AssistFightManager:getAchieveTotalExtAttr()
	local level = self:getAchieveLevel()
	local immuneRateTbl = {}
...
function AssistFightManager:checkIsStrategyMemberByFightType( gmid ,fight_type)
	...
	for pos,v in pairs(roleList) do
		if v == gmid then
			return pos
		end
	end
...
function AssistFightManager:checkRoleIsPossess(roleId)
	for cardRoleId in CardRoleManager.cardRoleList:iterator() do
	...
	for k,v in pairs(friendList) do
		...
		if info and info.provideRoles then
			...
			for _,roleId in pairs(provideRoles) do
				...
				if data.provideRole[idx].role.quality == 6 then
					data.provideRole[idx].maxTimes = 5
				elseif data.provideRole[idx].role.quality == 5 then
...
function AssistFightManager:getAssistRoleSuccess( event )
	...
	self.friendAssistList = self.friendAssistList or {}
