...
function RankManager:ctor(data)
	...
	--西岭噩梦榜
	TFDirector:addProto(s2c.SERDANG_RANK_REPONSE, self, self.getRankDataInfo_xiling)
...
function RankManager:RequestDataFromServer(type, start_index, length)
	...
	if timeDelay >= 60 then		
		if type == RankListType.Rank_List_CrossZhengBa then
			CrossZhengBaManager:requestOutRankInfo()
			return
		end
		local Msg = 
		{
...
--added by wuqi
function RankManager:getRankDataInfo_xiling(event)
	local rank_type = RankListType.Rank_List_Xiling
...
function RankManager:getTitlePic( index )
	...
	if MainPlayer:isOneYearBirth()  then
		return "ui_new/leaderboard/n11.png"
	end
