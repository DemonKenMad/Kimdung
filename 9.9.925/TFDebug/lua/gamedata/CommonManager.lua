...
function CommonManager:ctor()
    ...
    TFDirector:addProto(s2c.SERVER_GROUP_ID, self, self.onReceiveGroupId);
...
function CommonManager:showFightPluginErrorLayer()
    ...
    local pluginTimer = -1
    pluginTimer = TFDirector:addTimer(1000,1,nil,function ()
...
function CommonManager:showOperateSureLayer(okhandle,cancelhandle,param)
    ...
    if (param.isRich and param.isRich == true) then
        layer:setRichMsg(param.msg);
    else
...
function CommonManager:showOperateSureTipRefreshLayer(okhandle,cancelhandle,param)
    param = param or {}
    ...
    local layer = AlertManager:addLayerByFile("lua.logic.common.OperateSureTipRefresh",param.showtype,param.tweentype);
    layer.toScene = Public:currentScene();
    ...
    layer:setTitleImg(param.titleImg);
    --框框旁边的字
    layer:setfresh_tips(param.choicetips)
    layer:setBtnOkText(param.okText);
    layer:setBtnCancelText(param.cancelText);
    layer.Choicetext = param.msg
    layer.Unselectedtext = param.msg2
    AlertManager:show()

    return layer;
end

function CommonManager:showOperateCostSureTipLayer(okhandle,cancelhandle,param)
    param = param or {}

    param.showtype = param.showtype or AlertManager.BLOCK_AND_GRAY_CLOSE;
    param.tweentype = param.tweentype or AlertManager.TWEEN_1;

    param.uiconfig = param.uiconfig or "lua.uiconfig_mango_new.qiyu.DushiZhenXuan";


    local layer = AlertManager:addLayerByFile("lua.logic.qiyu.GambleZXSure",param.showtype,param.tweentype);
    layer.toScene = Public:currentScene();
    ...
    layer:setCostShow(param.costValue);
    layer:setCostPic(param.costType);
    if param.btnPic then 
        layer:setBtnPic(param.btnPic);
    end
...
function CommonManager:setRedPoint(parent,isshow,key,offset,functionId,scale)
    if parent then
...
function CommonManager:setRedPointMode( parent,_type ,offset)
    local redPoint = parent:getChildByName("RedPoint");
    if redPoint then
        if _type == 1 then
            redPoint:setTexture("ui_new/common/splats.png");
        elseif _type == 2 then
            if offset then
                offset.x = offset.x + 3
                offset.y = offset.y + 3
            end
            redPoint:setTexture("ui_new/home/icon_daoqi.png");
        end
        if offset then
            local pos = ccp(parent:getSize().width/2 - redPoint:getSize().width/2 + offset.x ,parent:getSize().height/2 - redPoint:getSize().width/2 + offset.y);
            redPoint:setPosition(pos);
...
function CommonManager:connectHandle(nResult,requestLogin)
    ...
    -- print("-- CommonManager:connectHandle MainPlayer.firstLoginMark = ", MainPlayer.firstLoginMark)
...
function CommonManager:sendLogin()
    ...
    if TFClientUpdate then
        sourceVersion = TFClientUpdate:GetUpdateDefaultVersion()
    end
...
function CommonManager:onReceiveGroupId( event )
    self.groupId = event.data.groupId
end

function CommonManager:getServerGroupId()
    return self.groupId
end

function CommonManager:IsOpenServerGroup()
...
function CommonManager:addGeneralHead( logic ,zOrder)
    ...
    logic.___generalHead_ = generalHead
    return generalHead;
end
...
--采纳海报
function CommonManager:openEverydayHaiBao()
    if (MainPlayer:getServerSwitchStatue(ServerSwitchType.HaiPao)) then
        AlertManager:addLayerByFile("lua.logic.home.HaiBaoLayer",AlertManager.BLOCK,AlertManager.TWEEN_0);
        AlertManager:show();
    end
end
...
function CommonManager:openWeixinDrawing()
    ...
    local time1     = GetCorrectTime({year=2015, month=11, day=20, hour=0})
    local time2     = GetCorrectTime({year=2015, month=11, day=26, hour=23, min=59})
    local bOpen     = false
...
function CommonManager:openKaiFuDaLiLayer()
    -- local daytime = math.ceil((GetCorrectTime() - MainPlayer:ServerOpenTime()) / (60*60*24))
    local daytime = SevenDaysManager:isOpenStatue()
    local _, leiJiInfo = SevenDaysManager:get14DaysLeiJiInfo()
    local dayInfo = SevenDaysManager:get14DaysJiangliInfo()

    -- 领取状态 0：未完成 1:已完成但未领取奖励  2:已完成并领取过奖励
    for _, v in ipairs(dayInfo) do
        if (v.state == 1) then
            isOpen = true
            break;
        end
    end
    for _, v in ipairs(leiJiInfo) do
        if (v.state == 1) then
            isOpen = true
            break;
        end
    end
    print("领取状态",isOpen,daytime)
    if (isOpen == true and daytime == true) then
...
function CommonManager:checkResourceVersion(loginCallBack)
...
function CommonManager:openIllustrationRole(roleId )
    ...
    local _layer = AlertManager:getLayerByName("lua.logic.role_new.RoleInfoLayer")
    if _layer then
        return
    end
...
function CommonManager:checkRedPointByKey( key, currTime )
    if self.checkRedPoint[key] == nil then
        self.checkRedPoint[key] = {}
        self.checkRedPoint[key].flag = false
        self.checkRedPoint[key].refreshTime = currTime + 5
        return true
    end
    if self.checkRedPoint[key].flag == false then
        return true
    end
...
function CommonManager:setRedPointStateByKey( key, state )
    if self.checkRedPoint[key] == nil then
        self.checkRedPoint[key] = {}
        self.checkRedPoint[key].flag = state
        self.checkRedPoint[key].refreshTime = 0
        return
    end

    self.checkRedPoint[key].flag = state

    print('setRedPointStateByKey = key = ', key)
    print('setRedPointStateByKey = state = ', state)
end

function CommonManager:getRedPointStateByKey( key )
    if self.checkRedPoint[key] and self.checkRedPoint[key].flag then
        print('getRedPointStateByKey == true')
        return true
    end
    print('getRedPointStateByKey == false')
    return false
end

function CommonManager:clearRedPoint()
    self.checkRedPoint = nil
    self.checkRedPoint = {}
end
