...
function TreasureManager:onConfigMessageCallBack(event)
    ...
    local scoreRewards = stringToNumberTable(data.scoreReward,'_')
    ...
    -- 解析寻宝消耗道具
    local tmpGolds = string.split(data.consumeSycee,'|')
    local goldLen = #tmpGolds
    if (goldLen < 3) then
        toastMessage(localizable.ActionError)
        return
    end
    local golds = {} 
    for i=1, goldLen, 1 do
        local tmpInfo = string.split(tmpGolds[i],'_') 
        table.insert(golds,tmpInfo)
    end

    print(self.bMeClick)
    if self.bMeClick then
        self:openTreasureLayer(count,configMessageList,golds,props,boxCounts,boxIndex,round,boxRewardList,actTime,isFirstFree,scoreRewards)
        self.bMeClick = false
    end
end
...
function TreasureManager:openTreasureLayer(count,ConfigMessage,golds,props,boxCounts,boxIndex,round,boxRewardList,actTime,freeTimes,scoreRewards)
    local layer = require("lua.logic.treasure.TreasureMain"):new()
    layer:loadData(count,ConfigMessage,golds,props,boxCounts,boxIndex,round,boxRewardList,actTime,freeTimes,scoreRewards)
    AlertManager:addLayer(layer,AlertManager.BLOCK_AND_GRAY,AlertManager.TWEEN_1)
    AlertManager:show()
end
