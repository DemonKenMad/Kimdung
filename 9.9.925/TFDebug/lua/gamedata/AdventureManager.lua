...
AdventureManager.changeLayerComplete = "AdventureManager.changeLayerComplete"
...
AdventureManager.OtherPlayerSwitch = false -- false关闭 true开启
...
function AdventureManager:ctor(data)
    ...
    --刷新随机事件
    TFDirector:addProto(s2c.ADVENTURE_EVENT_FRESH, self, self.onResetRandomEvent)      
...
function AdventureManager:requestHomeLayerData(dispatchMessage)
    local item = VersionLockData:objectByID(EnumVersionLockType.Adventure_Lock)
    if item and item.open == 0 then
        return 
    end
    local openLevel = FunctionOpenConfigure:getOpenLevel(2203)
    local currLevel = MainPlayer:getLevel()
    if openLevel > currLevel then
        return
    end
...
function AdventureManager:checkHaveNesRandomEvent( data )
    if self.mainStructData == nil then
        return false
    end
    local oldEvent = self.mainStructData.eventId or {}
    
    local currEvent = data.eventId or {}

    print("currEvent = ",currEvent)
    print("oldEvent = ",oldEvent)
    for k,v in pairs(currEvent) do
        local flag = true
        for old_k,old_v in pairs(oldEvent) do
            if old_v == v then
                flag = false
                break
            end
...
function AdventureManager:onHomeLayerDataReceive( event )
    ...
    print('onHomeLayerDataReceive event.data =============== ')
    print('onHomeLayerDataReceive event.data =============== ')
...
function AdventureManager:requestEventComplete( eventId )
    ...
    if eventId >= AdventureMissionManager.randomMissionIndexStart then
        --随机事件
        TFDirector:send(c2s.ADVENTURE_EVENT,{eventId, 0})
    else
        --主线任务        
...
function AdventureManager:startSecondBattle()
    print("startSecondBattle-------------------------------")
    --二次战斗不播放电影了
    self.bPlayMoive = true
    showLoading()
    TFDirector:send(c2s.START_SECOND_BATTLE,{})
end

function AdventureManager:isShowRedPoint()
    if self.mainStructData and self.mainStructData.eventId then
        local eventId = self.mainStructData.eventId or {}
        for _,v in pairs(eventId) do
            if v ~= 0 then
                return true
            end
        end
...
function AdventureManager:isGetYiWenLuExchange()
    if (MainPlayer:getServerSwitchStatue(ServerSwitchType.YiWenLu)) then
        if self.isYiWenLuExChangeState == 0 then
            return true
        end
...
function AdventureManager:isYouLiRedPoint()
    -- if self:isGetYiWenLuExchange() then
    --     return true
    -- end
    if AdventureTreasureManager:GetDigRedPoint() then
        return true
    end
    if AdventureTreasureManager:GetBuryRedPoint() then
        return true
    end
    return false
end

function AdventureManager:onResetRandomEvent( event )
    self.onResetRandomEventFlag = true
    self.mainStructData = self.mainStructData or {}
    self.mainStructData.eventId = event.data.eventId
    TFDirector:dispatchGlobalEventWith(AdventureManager.refreshRandomEvent, {})     
end

function AdventureManager:checkNeedToastMessage()
    if self.onResetRandomEventFlag then
        toastMessage(localizable.AdventureRandomEventTips)
    end
    self.onResetRandomEventFlag = nil
end
