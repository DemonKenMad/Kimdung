...
MultiServerFightManager.BETRATE                          = "MultiServerFightManager.betrate" --
MultiServerFightManager.REPONSEBetWinCount = "MultiServerFightManager.REPONSEBetWinCount"
...
function MultiServerFightManager:ctor(data)
    ...
    --押注比率
    TFDirector:addProto(s2c.REPONSE_CROSS_BET_RADIO_INFO, self, self.updataBetRate)
    --接收宝箱信息
    TFDirector:addProto(s2c.REPONSE_BET_WIN_COUNT, self, self.updatBetWinCount)
    self:restart()
end
...
--请求宝箱消息
function MultiServerFightManager:requestBetRate(roundId)
    showLoading()
    TFDirector:send(c2s.REQUEST_BET_WIN_COUNT, { roundId })
end
--请求押注比率
function MultiServerFightManager:BetRateInfo(roundId,index,betPlayerId)
    showLoading()
    local betMsg = {
        roundId,
        index,
    ...
    TFDirector:send(c2s.REQUEST_CROSS_BET_RADIO_INFO, betMsg)
end

--押注比率
function MultiServerFightManager:updataBetRate(event)
    hideLoading()
    TFDirector:dispatchGlobalEventWith(MultiServerFightManager.BETRATE,event.data)
end
function MultiServerFightManager:updatBetWinCount(event)
    hideLoading()
    if event.data then  
        self.boxes = event.data.boxes 
        self.betWinCount = event.data.winCount 
    end
    TFDirector:dispatchGlobalEventWith(MultiServerFightManager.REPONSEBetWinCount,{})
end
...
function MultiServerFightManager:openReportLayer(round,fightRound)
    local layer = AlertManager:addLayerByFile("lua.logic.multiServerFight.KuaFuRecordLayer")
    layer:setData(round,fightRound)
    AlertManager:show()
end

function MultiServerFightManager:getRecordList(round,fightRound)
    local recordList = {}
    if self.ChampionInfosWarData then
        for k,v in pairs(self.ChampionInfosWarData) do
            if v.round <= round then
                for j=1,3 do
                    local atkFlag = bit_and(v.atkWinRound,math.pow(2,(j-1)))
                    local defFlag = bit_and(v.defWinRound,math.pow(2,(j-1)))
                    if atkFlag ~= defFlag and ((v.round == round and j < fightRound) or v.round ~= round) then
                        local item = clone(v)
                        if atkFlag ~= 0 then
                            item.winPlayerId = item.atkPlayerId
                        else
                            item.winPlayerId = item.defPlayerId
                        end
                        item.replayId = nil
                        item.replayId = v.replayId[j]

                        item.forSortIndex = j*100 + v.index
                        item.forshowIndex = j
                        table.insert(recordList, item)
                    end
                end                
            end
        end     
    ...
    local function cmp(v1,v2)
        if v1.round == v2.round then
            return v1.forSortIndex < v2.forSortIndex
        else
            return v1.round < v2.round
        end
    end

    table.sort(recordList, cmp)
...
function MultiServerFightManager:requestBetWinCount(round)
    showLoading()
    TFDirector:send(c2s.REPONSE_BET_WIN_COUNT,{self.dispatchFightWarRound})    
end
...
function MultiServerFightManager:onCrossChampionInfosWar( event )
    ...
    if self.ChampionInfosWarData and self.ChampionInfosWarData[1] and self.ChampionInfosWarData[1].round then 
        self:requestBetRate(self.ChampionInfosWarData[1].round)
    end
    local layerFile = {
        "lua.logic.multiServerFight.KuaFuFirstRoundLayer",
        "lua.logic.multiServerFight.KuaFuSecondRoundLayer",
...
function MultiServerFightManager:getRoundAndStatus(fightStartTime, currTime)
    local currRound = 1 -- 三局两胜的第几场
    local currState = 1 -- 1正在战斗 2战斗结果预览
    
    local roundFight = ConstantData:objectByID("MultiServerFightManager.Tenminutes").value  --每场战斗时间
    local roundDelay = ConstantData:objectByID("MultiServerFightManager.Onehours").value --每场持续时间
    for i=1,3 do
        if currTime < (fightStartTime + i*roundDelay) then
            currRound = i
            if currTime < (fightStartTime + (i-1)*roundDelay + roundFight) then
                currState = 1
            else
                currState = 2
            end
    ...
    --当轮战斗持续时间
    local endTime = fightStartTime + currRound*roundDelay
    local fightEndTime = fightStartTime + (currRound - 1)*roundDelay + roundFight
    return currRound,currState,endTime,fightEndTime
end

function MultiServerFightManager:openBox(index)
    local box_id = self.boxes or 0
    ...
    if index == 4 then
        --跨服武林大会押注
        if  self.betWinCount < boxInfo.value then
    ...
    showLoading()
    TFDirector:send(c2s.OPEN_BOX, {4})
    self:requestBetRate(self.ChampionInfosWarData[1].round)
end

function MultiServerFightManager:openBoxInfo( boxInfo ,message)
...
function MultiServerFightManager:calculateReward(rewardid)
