...
function CardEquipment:init( Data )
	...
	self.orderAttribute  = GameAttributeData:new()				--升阶属性
	self.orderBreachLevel = 0		--装备升阶突破Level
	self.orderBreachPower = 0
	...
	self.immuneAttribute = EffectExtraData:new()
	self.immuneAttribute:clear()
	self.effectExtraAttribute = EffectExtraData:new()
	self.effectExtraAttribute:clear()
	self.beEffectExtraAttribute = EffectExtraData:new()
	self.beEffectExtraAttribute:clear()

	--added by wuqi
	self.starBasePercent	= 0									--升星基本属性加成百分比
...
--对解析装备表进行赋值操作
	...
	self.power  		= 0  										--战力
	self.special_equipment  		= equipMentData.special_equipment  					--是否为特殊装备
...
function CardEquipment:setOrderLevel( level, breachLevel, currExp )
	self.orderLevel = level
	self.orderAttribute:clear()	
	self.orderAttribute:init()
	self.immuneAttribute:clear()
	self.effectExtraAttribute:clear()
	self.beEffectExtraAttribute:clear()
	local attrList = EquipmentThroughAttribData:getTotalAttrByQualityLevel(self.equipType, self.quality, level) or {}
	for k,v in pairs(attrList) do
		self.orderAttribute:addAttr(k,v)
	end
	self.orderBreachLevel = breachLevel
	local specialInfo = EquipmentSpecialAttribData:getDetailsByQualityLevel(self.equipType, self.quality, breachLevel)
	if specialInfo then
		self.orderBreachPower = specialInfo.power
		if specialInfo.immune_rate and specialInfo.immune_rate ~= "" then
			self.immuneAttribute:add(GetAttrByString(specialInfo.immune_rate))
		end
		if specialInfo.effect_active and specialInfo.effect_active ~= "" then
			self.effectExtraAttribute:add(GetAttrByString(specialInfo.effect_active))
		end
		if specialInfo.effect_passive and specialInfo.effect_passive ~= "" then
			self.beEffectExtraAttribute:add(GetAttrByString(specialInfo.effect_passive))
		end
	else
		self.orderBreachPower = 0
	end

	self.orderCurrExp = currExp
end

function CardEquipment:getExtraPower()
	return self.orderBreachPower
end

function CardEquipment:setAddOrderExp( exp )
	self.orderCurrExp = self.orderCurrExp + exp
end

-- 装备升阶等级
function CardEquipment:getOrderLevel()
...
function CardEquipment:UpdateAttr()
	...
	for k,v in pairs(baseAttribute) do
		self.baseAttributeWithRecast:addAttr(k, math.floor(v*self.recastPercent/100) + math.floor(v * ( self:getStarAddAttrPercent() ) / 100 ))	
	end
	...
	--装备升阶属性
	self.totalAttr:setAddAttData(self.orderAttribute)
	self.totalAttrWithGem:setAddAttData(self.orderAttribute)
	newPower = newPower + CalculateEquipPower(self.orderAttribute.attribute)

	newPower = newPower + self.orderBreachPower
...
function CardEquipment:setStarAddPer(per)
	self.isViewByOther = true
	self.starAddPer = per
end
...
--获取升阶属性
function CardEquipment:getOrderLevelAttribute()	
	return self.orderAttribute
end

--获取升阶突破id 
function CardEquipment:getorderBreachLevel()
	return self.orderBreachLevel or 0
end

function CardEquipment:getCurrExp()
	return self.orderCurrExp or 0
end
--设置宝石
...
--added by wuqi
function CardEquipment:getStarAddAttrPercent()
	local tab = EquipStarAttrAddData.m_list
	table.sort(tab, function(a, b)
		return a.star < b.star
	end)
	local tab_new = {}
	for i = 1, #tab do
		tab_new[i] = tab[i].attrib
	end
	if (not self.equip) or (self.equip == 0) then
		return 0, nil, tab
	end	

	local role = CardRoleManager:getRoleById(self.equip)
	if role then
		local level = role:getStarAttrAddLevel()
		if level ~= 0 then
			return tab_new[level] / 100, level, tab
		end
	end
	return 0, nil, tab
end
...
function CardEquipment:isCanRecaseGoOnByPos(pos)
	...
	if info then
		if info.quality < EquipmentManager.kMaxRecastLevel then
			return true
		else
