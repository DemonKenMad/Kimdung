...
function EmployManager:getFilterList(filterType)
	if filterType == nil then
		filterType = 0
	end
	local filter_list = TFArray:new()
    if filterType == 0 or filterType == 1 then
	    for v in self.myEmployRoleList:iterator() do
	        local role = CardRoleManager:getRoleByGmid(v.roleId)
	        if role then
	            filter_list:pushBack(role.id)
	        end
	    end
    end
...
function EmployManager:isPlayerCanFiredByLevel( level )
	local gap_level = ConstantData:getValue("Employ.gap.Level")
	return MainPlayer:getLevel() + gap_level >= level
end
...
function EmployManager:openRoleList(clickCallBack,type)
