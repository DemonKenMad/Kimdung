...
--
local XiaYiData  		= require('lua.table.t_s_buy_xiayi')
...
MallManager.ReceiveBuyLongMenPoResult = "MallManager.ReceiveBuyLongMenPoResult"
--随机商店种类数量
...
function MallManager:ctor()
    ...
    TFDirector:addProto(s2c.LONG_MEN_BREAK_BUY_RESULT, self, self.onReceiveLongMenPoitemBuy)

    TFDirector:addProto(s2c.USED_XIAYI_BUY_COUNT, self, self.onReceiveBuyXiaYiCount)
    TFDirector:addProto(s2c.BUY_XIAYI_RESULT, self, self.buyXiaYiResultCallback)
    self.buyCoinNum = 0
    self.buyXiaYiNum = 0
...
--获取固定商店所有商品列表
function MallManager:getFixedStoreCommodityList(giftType)
	--giftType 1 限时礼包
	--giftType 2 普通礼包
	giftType = giftType or 1
	local list = TFArray:new()
	for v in ShopData:iterator() do
		if self:isSellBox(v) and v.type == giftType then
...
function MallManager:loadingCompleteCallback()
	local layer = AlertManager:addLayerByFile("lua.logic.mall.MallLayer",AlertManager.BLOCK_AND_GRAY);
    AlertManager:show()
    layer:showInfo(EnumMallType.NormalMall,self.defaultIndex or 1)
...
function MallManager:openYouFangLayer()
	...
	local layer = AlertManager:addLayerByFile("lua.logic.shop.YouFangLayer", AlertManager.BLOCK_AND_GRAY,tween)
	AlertManager:show()
end
...
function MallManager:buyXiaYiResultCallback(event)
	hideLoading()
	print("---购买侠义值  = ", event.data)
	-- required int32 consume = 1;				//消耗的元宝数量
	-- required int32 xiayi=2;					//获得侠义数量
	self.buyXiaYiNum = self.buyXiaYiNum + 1
	toastMessage(localizable.shop_xieYi_buy_succeed)
end
...
function MallManager:resetBuyXiaYiNum()
	self.buyXiaYiNum = 0
end
...
-- BaseDataManager:BindShopData()
	...
	giftItem.begin_time = GetCorrectDate("%Y/%m/%d %X",giftItem.begin_time)
	giftItem.end_time   = GetCorrectDate("%Y/%m/%d %X",giftItem.end_time)
...
function MallManager:getTravelBusinessVipPriceData()
	if not self.travelBusiness.discount then
		return nil
	end

	return self.travelBusiness.discount;
end
...
function MallManager:getShopByData( item ,type )
	for v in ShopData:iterator() do
        if v.res_id == item.res_id and v.res_type == item.res_type and (nil == type or v.type == type) then
            return v
        end
    end
    return nil
...
function MallManager:buyLongMenPo( resId , num )
	local msg = {
		resId,
		num
	}
	TFDirector:send(c2s.PURCHASE_ORDER_FOR_LONG_MEN_BREAK ,msg) 
end

function MallManager:onReceiveLongMenPoitemBuy( event )
	TFDirector:dispatchGlobalEventWith(MallManager.ReceiveBuyLongMenPoResult, {state = event.data.state,itemId = event.data.goodsId})
end

function MallManager:onReceiveBuyXiaYiCount( event )
	self.buyXiaYiNum = event.data.count

	print("self.buyXiaYiNum------ = ", self.buyXiaYiNum)
end

function MallManager:showBuyLMPItem(resId,func,closefunc)
	local layer = require('lua.logic.longmenpo.LongMenPoShoppingLayer'):new(resId)
	layer:setBuyClickCallFunc(func,closefunc)
	AlertManager:addLayer(layer,AlertManager.BLOCK_AND_GRAY,AlertManager.TWEEN_1)
	AlertManager:show()
end

function MallManager:buyXiaYi()
	showLoading();
	print("购买侠义值")
	TFDirector:send(c2s.REQUEST_COUNT_BUY_XIAYI ,{}) 
end

function MallManager:getByCountXiaYiData(count)
	--XiaYiData
	for info in XiaYiData:iterator() do
...
function MallManager:showBuyXiaYiItem(isAdd)
	local count 		= self.buyXiaYiNum + 1
	local xiaYiInfo 	= self:getByCountXiaYiData(count)
	local vipLevel 		= MainPlayer:getVipLevel()
	local maxBuyCount 	= 0
	local vipData  		= VipData:getVipItemByTypeAndVip(8450, vipLevel)
	if (vipData) then
		maxBuyCount 	= maxBuyCount + (vipData.benefit_value or 0)
	end

	local buyCount 		= maxBuyCount - self.buyXiaYiNum
	if (buyCount <= 0) then
		local nextData 	= VipData:getVipItemByTypeAndVip(8450, vipLevel + 1)
		if (nextData and nextData.benefit_value > maxBuyCount) then
            --"今日购买次数已用完！\n\n提升至{v1}可购买{p2}次。\n\n是否前往充值？"
            local str = stringUtils.format(localizable.CommonManager_need_up_vip, vipLevel + 1, nextData.benefit_value)
            CommonManager:showOperateSureLayer(
                    function()
                    ...
                    {
	                    title 		= localizable.shop_xieYi_title1,
	                    msg 		= str,
        ...
        elseif(isAdd) then
        	toastMessage(localizable.shop_xieYi_buy)
        	return true
		end
		return false
	end
	local strTitle = localizable.shop_xieYi_title1
	if (isAdd) then
		strTitle 	= localizable.shop_xieYi_title2
	end
	if (xiaYiInfo == nil) then
		toastMessage(localizable.shop_xieYi_buy)
		return
	end
	local message = stringUtils.format(localizable.shop_xieYi_tips, xiaYiInfo.sycee, xiaYiInfo.xiayi, buyCount)    
    CommonManager:showOperateSureLayer(
            function(data, widget)
                self:buyXiaYi()
            end,
