...
--装备最高星级
EquipmentManager.kMaxUpgradeLevel   		= 15 --装备最高升阶等级
EquipmentManager.kMaxStarLevel   			= 10
EquipmentManager.kMaxStarLevelOld			= 5
EquipmentManager.kMaxRecastLevel			= 8

--装备开放新星级等级
EquipmentManager.kTenStarNeedLevel = 100
...
EquipmentManager.kGemMergeMaxLevel = 15
--宝石合成目标宝石索引
...
--装备升阶增加的属性条数
EquipmentManager.orderAddAttrNum = 3

--[[ 疯掉了的东西]]
EquipmentManager.SmithyBase_Intensify = 1
EquipmentManager.SmithyBase_StarUp = 2
EquipmentManager.SmithyBase_Practice = 3
EquipmentManager.SmithyBase_Refining = 4
EquipmentManager.SmithyBase_Recast = 5
EquipmentManager.SmithyBase_Upgrade = 6
EquipmentManager.SmithyBase_Gem = 7
EquipmentManager.SmithyBase_GemBuild = 8

EquipmentManager.EquipFromSelf = 1
EquipmentManager.EquipFromOthen = 2
...
--[[
    ...
    if verLock and verLock.open == 0 then
        self.kMaxStarLevel = self.kMaxStarLevelOld
    end
    self.isHaveEquip = nil
end

--added by wuqi
function EquipmentManager:initQualityMap()
	self.qualityMap = 
	{
		--品质,部位
		[1] = {},
	...
	for v in self:iterator() do
		if v.equip == nil or v.equip == 0 then						
			local type = v.equipType
			local quality = v.quality
			if not self.qualityMap[quality][type] then
				self.qualityMap[quality][type] = {}
			end

			table.insert(self.qualityMap[quality][type], v)
		end
	end
end

function EquipmentManager:isQualityMapEmpty(index)
	local empty = true
	if not index then		
		for i = 1, 6 do
			for j = 1, 5 do
				if self.qualityMap[i][j] then
					empty = false
				end
			end
		end
		...
		for i = 1, 6 do
			if self.qualityMap[i][index] and #self.qualityMap[i][index] ~= 0 then
				empty = false
			end
		end
	end
...
local function sortlistByQuality(v1,v2)
	if v1.quality ~= v2.quality then
		return v1.quality > v2.quality  -- 先按品质排序
	end

	if v1:getpower() ~= v2:getpower() then
		return v1:getpower() > v2:getpower()  --再按战力排序
	end

	return v1.star > v2.star  --最后按star排序
end 

local function sortlistByType(v1,v2)
	if v1.equipType ~= v2.equipType then
		return v1.equipType < v2.equipType
	end
	...
	return v1:getpower() < v2:getpower()
end
...
function EquipmentManager:GetEquipByTypeAndUsed(equipType,isused,sortDisable,isSpecial)
	local EquipArray = TFArray:new()
	for v in self:iterator() do
		if equipType == nil or v.equipType == equipType or equipType == 0 then
		    if isused and v.equip and v.equip ~= 0 then
		    	if isSpecial == nil or
		    	 		(isSpecial == false and v.special_equipment == 0) or 
		    			(isSpecial == true and v.special_equipment == 1) then
					EquipArray:push(v)
				end
...
--[[--
	返回指定装备ID的装备数量
	@param equipType: 装备ID
]]	
function EquipmentManager:GetEquipByIdToCount(equipId)
	local count1 = 0
	local count2 = 0
	for v in self:iterator() do
		if (v.id == equipId) then
			if (v.equip == nil or v.equip == 0) then
				count1 = count1 + 1
			elseif (v.equip and v.equip ~= 0) then
				count1 = count1 + 1
			end
		end
	end
...
function EquipmentManager:GetOtherEquipByUsed(gmid,isused,sortDisable)
	...
	for v in self:iterator() do
		--有重铸过的装备排除
		--周年装备排除
		if v.recastPercent <= 0 and v.special_equipment ~= 1 then
			if isused and v.equip and v.equip ~= 0 then
				if v.gmId ~= gmid then
					EquipArray:push(v)
				end
...
-- }
	...
	if  data.success then
		...
		if role then
			for v in role:getEquipment():iterator() do
				v:UpdateAttr()
			end
		end
...
EquipmentManager.Function_UpGrade = 2211
--[[
验证装备强化是否开放
...
function EquipmentManager:getEquipListByID(id, except_gmid, except_special)
	local EquipArray = TFArray:new()
	for v in self:iterator() do
		if (v.id == id and v.gmId ~= except_gmid) and 
			(except_special == nil or except_special == false or v.special_equipment == 0) and
			(v.equip == nil or v.equip == 0) then
			EquipArray:push(v)
		end
	end
...
function EquipmentManager:getEquipNumberByID(id)
	local count = 0
	for v in self:iterator() do
		if (v.id == id) then
			count = count + 1
...
--是否开放10星
function EquipmentManager:checkIsOpenTenStar(equip)
	if not equip then
		return
	end
	local verLock = VersionLockData:objectByID(EnumVersionLockType.EquipUpTen_Lock)
	...
	if equip.level >= EquipmentManager.kTenStarNeedLevel then
		return true
	end
	return false
end
...
--装备升阶
function EquipmentManager:equipIncreaseExp(gmId, data)
	...
	Msg[2] = gmId
	print('Msg = ',Msg)
	TFDirector:send(c2s.INCREASE_STEPS_EXP,Msg)
end
--装备升阶
function EquipmentManager:equipStepsThrough(gmId)
	showLoading()	
	local Msg = {
		gmId
	}
	TFDirector:send(c2s.STEPS_THROUGH,Msg)
end

function EquipmentManager:getIsHaveEquip(index,brush)
	if brush or self.isHaveEquip == nil then
		self.isHaveEquip = {}
		for i=1,5 do
	        local equipList = self:GetEquipByTypeAndUsed(i,false);
	...
	return self.isHaveEquip[index]
end

function EquipmentManager:checkPVEStrategyEquip()
	local fightMatix = StrategyManager:getList()
	KaojiaoManager:checkEquipXiangqianByEquipId(nil,true)
	for i = 1,10 do
		if fightMatix[i] and fightMatix[i] ~= 0 then
			local role = CardRoleManager:getRoleByGmid(fightMatix[i])
			if role then
				for j = 1, 5 do
		            local equipInfo = role:getEquipment():GetEquipByType(j)
		            if equipInfo then
			            ...
			            local bxiangqian = KaojiaoManager:checkEquipXiangqianByEquipId(equipInfo.gmId,false)
			            if bxiangqian == true then
			            	return true
			            end
		            end
		        end
...
function EquipmentManager:getSuitEquipInfo( roleId, fromType )
	local SuitEquipInfo = {}
	local role = nil
	if fromType == EquipmentManager.EquipFromOthen then
		role = OtherPlayerManager:getRoleByid(roleId)
	else
		role = CardRoleManager:getRoleByGmid(roleId)
	end
	
	if role == nil then 
		return
	end
	local listEquip = role:getEquipment()
	for _, equip in pairs(listEquip.map) do
		local suit = EquipmentTemplateData:objectByID(equip.id)
		if (suit.suit_id ~= 0 and suit.suit_id ~= nil) then
			local suitId = suit.suit_id
			if (SuitEquipInfo[suitId]) then
				SuitEquipInfo[suitId] = SuitEquipInfo[suitId] + 1
			else
				SuitEquipInfo[suitId] = 1
			end
		end
	end
	return SuitEquipInfo
end
