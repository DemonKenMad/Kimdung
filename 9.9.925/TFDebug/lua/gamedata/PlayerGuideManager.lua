...
local tTriggerFunctions = {
	...
	["smallPacket"]	 = tGetTriggerFunction.getSmallPacketResource,					--是否是小包资源且有未下载资源
}
...
function PlayerGuideManager:playDialogEffect( effect , effectTime )
	if self.effectHandle ~= nil then
		TFAudio.stopEffect(self.effectHandle)
		self.effectHandle = nil
	end
...
function PlayerGuideManager:stopSound()
	if self.effectHandle ~= nil then
		TFAudio.stopEffect(self.effectHandle)
		self.effectHandle = nil
	end
...
function PlayerGuideManager:triggerGuide( guideInfo)
	...
	for i, v in pairs(tTriggerFunctions) do
		if guideInfo.conditions[i] and not TFFunction.call(v, nil, guideInfo.conditions) then
			return false
		end
	end
	return true
...
function PlayerGuideManager:showGuideLayerByStepId( stepId ,save)
	...
	if guideStepInfo.guideType == 0 then
		...
		self:showGuideLayer(guideStepInfo,save, isNotPlaySound)
	...
	elseif guideStepInfo.guideType == 3 then
		self:showDialog(guideStepInfo, save)
	end
...
function PlayerGuideManager:doGuide()
	...
	if self.next_step ~= 0 and not self.isUp then
...
function PlayerGuideManager:isGuideFunctionOpen(functionId)
	...
	if (not MainPlayer.guideList or not functionId) then
		return false
	end
...
function PlayerGuideManager:getGuideForcePanelAll()
	...
	guideForcePanel:setBackGroundColorOpacity(80)
...
--通过widget来添加控件黑层
function PlayerGuideManager:getGuideForcePanelbyWidget( panelBg  , guideInfo,widget_name  )
	local topLayer = self:getTopLayer()
	if widget_name == nil  then
		widget_name = guideInfo.widget_name
	end
	if widget_name and widget_name ~= "" then
...
--闯关前几关常驻引导
function PlayerGuideManager:addLongTimeGuide(layer, widget, type, bAdd)
	local tempLayer = layer:getChildByName("longTimeGuideLayer" .. type)
	if tempLayer then
		tempLayer:removeFromParent(true)
		tempLayer = nil
	end 
	if (not bAdd) or (not widget) then
		return
	end
	local emptyPanel = TFPanel:create()
	emptyPanel:setName( "longTimeGuideLayer" .. type )
	emptyPanel:setZOrder(200)
	--emptyPanel:setPosition(ccp(0, 0))
	layer:addChild(emptyPanel)

	local pos = ccp(0, 0)
	if widget.getParent and widget:getParent() then
		pos = widget:getParent():convertToWorldSpaceAR(widget:getPosition())
	else
		pos = layer:convertToWorldSpaceAR(widget:getPosition())
	end

	-- local glView = CCDirector:sharedDirector():getOpenGLView()
	-- local frameSize = glView:getFrameSize()
	-- local designSize = glView:getDesignResolutionSize()
	-- print("size1 == ", frameSize)
	-- print("size2 == ", designSize)
	-- pos.x = (designSize.width - 960) + pos.x

	local anchorPoint = widget:getAnchorPoint()
	local pos_x = pos.x - anchorPoint.x * widget:getSize().width
	local pos_y = pos.y - anchorPoint.y * widget:getSize().height
	...
	local kuangList = {}
	local delta = 15
	kuangList[1] = self:addGuideKuang(ccp(pos_x + width+delta, pos_y + height+delta),ccp(0,1))
	kuangList[1]:setScaleX(-1)

	kuangList[2] = self:addGuideKuang(ccp(pos_x-delta,pos_y + height+delta),ccp(0,1))

	kuangList[3] = self:addGuideKuang(ccp(pos_x + width+delta, pos_y-delta),ccp(0,1))
	kuangList[3]:setScaleY(-1)
	kuangList[3]:setScaleX(-1)

	kuangList[4] = self:addGuideKuang(ccp(pos_x-delta, pos_y-delta),ccp(0,1))
	kuangList[4]:setScaleY(-1)

	for i = 1, 4 do
		emptyPanel:addChild( kuangList[i] )
	end
	emptyPanel:setScaleX(layer:getScaleX())
	self:showhandEffect(emptyPanel, {}, pos)
end

--当新手引导和常驻引导冲突
function PlayerGuideManager:shouldLongTimeGuideShow(index)
	index = index or 1
	local bShow = true
	--local guideInfo = PlayerGuideStepData:objectByID(self.next_step) or PlayerGuideStepData:objectByID(self.now_step) or {}
	local guideInfo = PlayerGuideStepData:objectByID(self.next_step) or {}
	-- print("step1 == >> ", self.next_step)
	-- print("step2 == >> ", self.now_step)
	local name = guideInfo.layer_name
	if name then
		--if ( index == 1 and name == "MenuLayer" ) or ( index == 2 and name == "MissionLayer" ) then
		if index == 1 and name == "MenuLayer" then
			bShow = false
		end
	end
	local topLayer = self:getTopLayer()
	if topLayer and topLayer.__cname ~= "MenuLayer" then
		bShow = false
	end
	return bShow
end
...
-- 创建指引强制黑层Panel
function PlayerGuideManager:getGuideForcePanel(panelBg  , guideInfo,widget_name )
	...
	if self:getGuideForcePanelbyWidget(panelBg,guideInfo,widget_name ) then return end
	local guideForcePanel = self:getGuideForcePanelAll()
	if guideForcePanel then
		panelBg:addChild(guideForcePanel)
		panelBg.layerList = panelBg.layerList or {}
...
--显示强制指引黑层
function PlayerGuideManager:showForceLayer(guidePanel_bg ,guideInfo,widget_name )
	if guideInfo.force == false then

	else
		self:getGuideForcePanel(guidePanel_bg , guideInfo,widget_name )
	end
end
...
--显示动画或者特效
function PlayerGuideManager:showEffect(panelbg, guideInfo ,widget_name )
	...
	if guideInfo.tip ~= nil and guideInfo.tip ~= "" then
		...
		if guideInfo.bgImgName == nil then
			if guideInfo.onlybox == true then
				if textLen <= 26 then
					bgImgName = "ui_new/guide/img_guide.png"
				else
					bgImgName = "ui_new/guide/img_guide2.png"
				end
			else
				if textLen <= 26 then
			...
			bgImgName = "ui_new/guide/"..guideInfo.bgImgName..".png"
		end
		...
		if guideInfo.onlybox == true then
			if textLen <= 26 then
				if textLen <= 13 then
					textLabel:setPosition(4, -12)
				else
					textLabel:setPosition(4, -4)
				end
				textLabel:setTextAreaSize(CCSizeMake(270,55))
			else
				if textLen <= 39 then
					textLabel:setPosition(4, -12)
				else
			...
			if textLen <= 26 then
				if textLen <= 13 then
					textLabel:setPosition(16, -100)
				else
					textLabel:setPosition(16, -90)
				end
				textLabel:setTextAreaSize(CCSizeMake(270,55))
			else
				if textLen <= 39 then
					textLabel:setPosition(22, -76)
				else
					textLabel:setPosition(22, -64)
				end
				textLabel:setTextAreaSize(CCSizeMake(270,95))
		    end
		end
...
function PlayerGuideManager:panelbgClick( guideInfo )
	if self.m_guideInfo.node == nil or  self.m_guideInfo.node.guidePanel_bg == nil then
		return
	end
	local topLayer = self:getTopLayer()
	if topLayer == nil then	return end
	...
	function()
		...
		local layer = self:getGuidePanelBg()
		local child = widget:getChildByTag(557)
		if child and child.tipList and child.tipList.guideType == 3 then
			print("child.tipList.guideType == 3", child.bTextFlyEnd)
			if not child.bTextFlyEnd then
				child.bTextFlyEnd = true
				child:skipActions()
				return
			end
		else
		...
		if widget then
			...
			if clickHandleFunction then
				...
				function()
					...
					self:removePanelBg()
					if self.now_step ~= 10000 then
						self.next_step = guideInfo.next_step
					end
					print("PlayerGuideManager:forceClick( guideInfo 111111111111)",self.next_step,self.now_step)
...
-- 显示指引的所有层
function PlayerGuideManager:showAll( guideInfo ,save , isNotPlaySound)
	local topLayer = self:getTopLayer()
	if topLayer == nil then
		return
	end
	...
	if not isNotPlaySound and guideInfo.sound and guideInfo.sound ~= "" then
...
-- 显示指引
function PlayerGuideManager:showGuideLayer(guideInfo ,save, isNotPlaySound)
	local topLayer = self:getTopLayer()
	if topLayer == nil then
		return
	end
	...
	self:showAll(guideInfo,save, isNotPlaySound)
end

--新加对话指引
function PlayerGuideManager:showDialog(guideInfo, save)
	local topLayer = self:getTopLayer()
	if topLayer == nil then
		return
	end
	...
	self:beginCallBack(guideInfo)
	self:saveGuideStep(save)
	self:showDialogLayer(guideInfo, save)
end

--显示对话层
function PlayerGuideManager:showDialogLayer(guideInfo, save)
	local topLayer = self:getTopLayer()
	if topLayer == nil then
		return
	end
	...
	--遮罩panel
	self:showForceLayer(guidePanel_bg, guideInfo)
	self:showDialogContent(guidePanel_bg, guideInfo)
	-- if self.now_functionId  >= 1 and self.now_functionId <= 20 then
	-- 	self.buttonTimerId = TFDirector:addTimer(TiaoguoTime*1000, 1, nil,
	-- 	function()
	-- 		if self:getTopLayer() ~= topLayer then
	...
	if guideInfo.sound and guideInfo.sound ~= "" then
		self:playDialogEffect(guideInfo.sound ,guideInfo.soundTime*1000);
	end
	self:setClickEvents(guideInfo)
...
function PlayerGuideManager:showDialogContent(guidePanel_bg, guideInfo)
    local tipData = {}
    local tipList = PlayerGuideStepData:objectByID(guideInfo.id)
    tipData.tiplist = tipList
    local tipLayer = require("lua.logic.mission.MissionTipLayer"):new(tipData, true)
    tipLayer.tipList = tipList
    tipLayer:setTag(557)
    guidePanel_bg:addChild(tipLayer);
end
...
function PlayerGuideManager:activityLayerSpecial(index)
	local topLayer = self:getTopLayer()
	if topLayer and topLayer.__cname == "ActivityLayer" then
		topLayer:setTableViewTop(index)
	end
end
...
function PlayerGuideManager:armyLayerPutSpecial(guideInfo)
    ...
    -- print("fick -------------------")
	local nTimerId = -1
	nTimerId = TFDirector:addTimer(0, 1, nil,
		function()
...
function PlayerGuideManager:armyLayerMoveSpecial(guideInfo)
	...
	if cardRole == nil or cardRole.outline ~= 2 then
		...
		local fromPos = guideInfo.fromPos or 2
		local changPos = guideInfo.changPos or 4
		topLayer:setChangePosGuide(fromPos, changPos)
	end

	local _updatePosCallBack = function(event)
		TFDirector:removeMEGlobalListener("armyLayerMoveSpecial", _updatePosCallBack );
    	self:_armyLayerMoveSpecial( guideInfo, topLayer)
    end;
    TFDirector:addMEGlobalListener("armyLayerMoveSpecial" ,_updatePosCallBack ) ;
    if StrategyManager:getRoleByIndex(4) ~= nil then
    	TFDirector:removeMEGlobalListener("armyLayerMoveSpecial", _updatePosCallBack );
    	self:_armyLayerMoveSpecial( guideInfo, topLayer)
    end
end

function PlayerGuideManager:_armyLayerMoveSpecial( guideInfo,topLayer)
    self:armyLayerPutSpecialEnd()
    self.next_step = guideInfo.next_step
    if guideInfo.save then
		self:saveGuideStep(guideInfo.save)
...
function PlayerGuideManager:beginCallBack(guideInfo)
	...
	elseif special == 9 then                             --武林界面最左
		self:activityLayerSpecial(1)
	elseif special == 10 then                             --武林界面最右
		self:activityLayerSpecial(2)
	end
end
...
function PlayerGuideManager:ShowGameBeginVideo(ui)
	...
	function()
		...
		if click_time ~= 0 and time - click_time < 500 and isCkick == false then
			if nTimerId then
				TFDirector:removeTimer(nTimerId)
				nTimerId = nil
			end
...
function PlayerGuideManager:isGuideNowByName(name)
	...
	if guide and self.now_functionId == guide.id and self.next_step ~= 0 then
		return true
	end
	return false
end
...
function PlayerGuideManager:showAnimation( guideInfo )
	...
	local timerId = -1
	timerId = TFDirector:addTimer(1,1,nil,function ()
...
function PlayerGuideManager:doGuideByGuideID(guideid)
	...
	if self:isGuideFunctionOpen(guideid) then
		return false
	end
	if self.next_step ~= 0 then
		return
...
--  新手指引 播放动画及战斗
function PlayerGuideManager:ShowGameBeginVideo2(ui)
	if tolua.isnull(ui) then
		return false
	end
	if not self.bOpenGuide then
	...
	video:setAnchorPoint(ccp(0.5, 0.5))
	video:setPosition(ccp(nViewWidth/2, nViewHeight/2))
	video:setFileName("effect/gamebegin2.mp4")
	video:play()
	self.isPlayGuideVideo = true
	video:setTouchEnabled(true)
	video:addMEListener(TFVIDEO_COMPLETE, 
...
--通过步骤信息显示指引
function PlayerGuideManager:showFightGuideLayer( stepId ,widget_name)
	local topLayer = self:getTopLayer()
	if topLayer == nil then
		return
	end
	...
	if widget_name and widget_name ~= "" then
		...
		if widget == nil then
			print("showGuideLayerByStepId widget == nil",topLayer.__cname)
			-- self:delayShowGuideLayerIfWidgetNil(stepId ,false)
			return
		end
	end
	self:_showFightGuideLayer(guideStepInfo , widget_name)
end


-- 显示指引
function PlayerGuideManager:_showFightGuideLayer(guideInfo,widget_name )
	local topLayer = self:getTopLayer()
	if topLayer == nil then
		return
	end
	...
	self.m_guideInfo.node = {}
	self:beginCallBack(guideInfo)
	self:showFightGuideAll(guideInfo,widget_name)
end

-- 显示指引的所有层
function PlayerGuideManager:showFightGuideAll( guideInfo,widget_name )
	local topLayer = self:getTopLayer()
	if topLayer == nil then
		return
	end
	...
	topLayer:addChild(guidePanel_bg)
	self.m_guideInfo.node.guidePanel_bg = guidePanel_bg
	self:showForceLayer(guidePanel_bg ,guideInfo,widget_name )
	self:showEffect(guidePanel_bg ,guideInfo,widget_name )
	
	if guideInfo.sound and guideInfo.sound ~= "" then
		self:playEffect(guideInfo.sound ,guideInfo.soundTime*1000);
	end
	self:setFightGuideClickEvents(guideInfo,widget_name)
end


--设置点击事件
function PlayerGuideManager:setFightGuideClickEvents(guideInfo,widget_name)
	print("PlayerGuideManager:setFightGuideClickEvents(guideInfo)",widget_name)
	if guideInfo.force == false then
		self:unFightGuideForceClick(guideInfo,widget_name)
	else
		self:fightGuideForceClick(guideInfo,widget_name)
	end
end


--非强制的事件
function PlayerGuideManager:unFightGuideForceClick( guideInfo ,widget_name)
	if guideInfo == nil then return end
	local topLayer = self:getTopLayer()
	if topLayer == nil then	return end
	if widget_name ~= "" then
...
--强制的事件
function PlayerGuideManager:fightGuideForceClick( guideInfo ,widget_name)
	if guideInfo == nil then return end
	local topLayer = self:getTopLayer()
	if topLayer == nil then	return end
	local layer_name = topLayer.__cname
