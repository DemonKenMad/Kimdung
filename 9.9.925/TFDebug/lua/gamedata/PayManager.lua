...
PayManager.refreshVIPShop = "PayManager.REFRESH_VIP_SHOP"

PayManager.IsAuthenticateUser = false
...
function PayManager:ctor()
    ...
    TFDirector:addProto(s2c.VIPTRUST, self, self.onReceiveVIPInfo);

    -- s2c.REPONSE_VIP_SHOP = 0x4a01 获取贵宾商城信息
    TFDirector:addProto(s2c.REPONSE_VIP_SHOP, self, self.onReceiveVIPShopInfo);
     
    -- 
    self.firstChargeStatus = false -- 首冲没有被领取

    -- if CC_TARGET_PLATFORM == CC_PLATFORM_IOS and not TFDeviceInfo:getIsJailBreak() then
    ...
    for recharge in self.rechargeList:iterator()  do
        if recharge.is_show == 1 then
            self.rechargeNum = self.rechargeNum + 1
        end
    end
...
function PayManager:onReceivePayComplete( event )
    ...
    if rechargeId == 7 then
        ...
        textMsg = stringUtils.format(localizable.PayManager_monthCard_desc,  localizable.PayManager_monthCard_name1, rechargeValue, cardInfo.YB)
    ...
    elseif rechargeId == 11 then
        ...
        -- textMsg = textMsg..[[</font>]]
        -- textMsg = textMsg..[[</p>]]        
        textMsg = stringUtils.format(localizable.PayManager_monthCard_desc, localizable.PayManager_monthCard_name2,  rechargeValue, cardInfo.YB)
    end
    if textMsg then
    ...
    if baseValue and baseValue > 0 then
        ...
        local ticketGetNum = 0
        if event.data.numForRechargeTicket then
            ticketGetNum = event.data.numForRechargeTicket
        end
        self:showPayResult(baseValue,showValue, not event.data.multiple, ticketGetNum)
    end
...
function PayManager:getSyceeNumForPayCompelete(isFirstPay,rechargeId)
    ...
    if self:isHavePay(rechargeItem.id) then
        ...
        return rechargeItem.sycee * (ConstantData:getValue("Recharge.First.Multiple")+1) + rechargeItem.extra_sycee ;
    end
...
function PayManager:getSyceeNumDetail(isFirstPay,rechargeId)
    ...
    if rechargeItem.id ==7 or rechargeItem.id == 11 then
        ...
        extValue  = MonthCardManager:getExtValue(rechargeItem.id,rechargeItem.extra_sycee)
    elseif self:isHavePay(rechargeItem.id) then
        ...
        extValue  = rechargeItem.sycee * ConstantData:getValue("Recharge.First.Multiple")
    end
...
--vip商城请求协议
function PayManager:updateVipShopInfo()
    showLoading();
    -- c2s.REQUEST_VIP_SHOP = 0x4a01 获取贵宾商城信息
    TFDirector:send(c2s.REQUEST_VIP_SHOP, {} );
end

function PayManager:sendVipShoppingData(goodId, number)
    showLoading();
    -- c2s.BUY_IN_VIP_SHOP = 0x4a02 购买
    TFDirector:send(c2s.BUY_IN_VIP_SHOP, {goodId, number} );
end

function PayManager:sendVipShopInfo()
    showLoading();
    -- c2s.REFRESH_VIP_SHOP = 0x4a03 刷新
    TFDirector:send(c2s.REFRESH_VIP_SHOP, {} );
end
...
function PayManager:showVipShopLayer(tween)
    self:updateVipShopInfo()
    tween = tween or AlertManager.TWEEN_1;
    AlertManager:addLayerToQueueAndCacheByFile("lua.logic.pay.VipShopLayer",AlertManager.BLOCK_AND_GRAY,tween);
    AlertManager:show();
...
function PayManager:showPayResult(baseValue, extValue,showMultiple, ticketGetNum)
    ...
    if ticketGetNum and ticketGetNum > 0 then
        local function closeCallBack()
            local layer = AlertManager:addLayerToQueueAndCacheByFile("lua.logic.pay.TicketSuccessLayer",AlertManager.BLOCK_AND_GRAY_CLOSE,tween)
            layer:setValue(ticketGetNum)
            layer:setIcon(self.index)
            AlertManager:show()
        end
...
function PayManager:onReceiveDoubleRechargeInfo(event)
    ...
    for i=1,#data.list do
        ...
        self.doubleRechargeInfo[info.index] = (info.multiple-100)/100
    end
    -- self.doubleRechargeInfo = data.list
end
function PayManager:onReceiveVIPInfo(event)
    if (event.data.staus == 1) then
        PayManager.IsAuthenticateUser = true
    end

end

function PayManager:onReceiveVIPShopInfo(event)
    hideLoading()
    -- s2c.REPONSE_VIP_SHOP = 0x4a01 获取贵宾商城信息
    -- print("s2c.REPONSE_VIP_SHOP ",event.data)
    local data = event.data
    self.vipShopInfo = {}
    for i = 1, 10 do 
        self.vipShopInfo[i] = {}
        local goodsData = VipShopItemsData:getGoodsInfoById(data.detil[i].goodId)
        self.vipShopInfo[i].id = data.detil[i].goodId
        self.vipShopInfo[i].itemId = goodsData.goods_id
        self.vipShopInfo[i].number = goodsData.num
        self.vipShopInfo[i].type = EnumDropType.GOODS
        --商品折扣
        self.vipShopInfo[i].discount = data.detil[i].discount * 0.1 --折扣服务器发过来 1折是10,2折是20
        --商品单价
        self.vipShopInfo[i].price = goodsData.price
        --商品折扣价
        self.vipShopInfo[i].cost = goodsData.price * (data.detil[i].discount * 0.01)
        --剩余购买次数
        self.vipShopInfo[i].purchaseNum = data.detil[i].number
    end

    self.vipIntegral = data.vipScore
    TFDirector:dispatchGlobalEventWith(self.refreshVIPShop, {})
end
...
function PayManager:isOpenFirstPay()
    return self.openFirstPay
end
--是否显示vip商城
function PayManager:isOpenVIPShop()
    if (MainPlayer:getVipLevel() >= 1 and MainPlayer:getServerSwitchStatue(ServerSwitchType.VipShop)) then
        return true
    end
    return false
end
function PayManager:getVipShopData(idx)
    if (idx and self.vipShopInfo[idx] ~= nil) then
        return self.vipShopInfo[idx]
    end
    return self.vipShopInfo
end
function PayManager:getVipIntegral()
    return self.vipIntegral
end
