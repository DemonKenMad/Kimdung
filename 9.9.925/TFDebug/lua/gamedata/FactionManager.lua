...
GuildDynamicType = {
	...
	RED_PACKET_RECEIVE = 25, --领取帮派红包
	SEND_RED_PACKET = 26, --发送帮派红包
	GUILD_BOSS_TRIGGER = 27, --帮派演武触发
	GUILD_BOSS_START = 28 --帮派演武开启
}
...
function FactionManager:ctor(data)
	...
	--祭拜记录
	TFDirector:addProto(s2c.WORSHIP_RECORDS, self, self.onWorshipRecodeSucc)

	--弹劾投票
	TFDirector:addProto(s2c.IMPEACHMENT, self, self.onImpeachmentSucc)
...
function FactionManager:openFactionHomeLayer()
	PeakManager:requestFactionRank(1)
	...
	AlertManager:show() 
	print("打开帮派主界面",self.factionInfo.state)
	-- 如果帮主属于被弹劾中,请求投票
	if (self.factionInfo.state == 3) then
		TFDirector:send(c2s.IMPEACHMENT_MSG_REQUEST,{})
	end
end
--打开帮派宣言或者公告的编辑界面
...
function FactionManager:factionRecordReceive( event )
	...
	local currDate = GetCorrectDate("*t", math.floor(msgTab[1]/1000))
	...
	if info.type == GuildDynamicType.WORSHIP then
		...
		if drinktype == 3 then
			...
			playerInfo.starHeroRanking = msgTab[14]
			playerInfo.ladderGrade = msgTab[15]
	...
	elseif info.type == GuildDynamicType.RED_PACKET_RECEIVE then
		local receiverName = msgTab[2]
		local senderName = msgTab[3]
		local goldNum = tonumber(msgTab[4])
		textSub = stringUtils.format(textTemplete,receiverName,senderName,goldNum)
	elseif info.type == GuildDynamicType.SEND_RED_PACKET then
		local senderName = msgTab[2]
		local goldNum = tonumber(msgTab[3])
		textSub = stringUtils.format(textTemplete,senderName,goldNum)
	elseif info.type == GuildDynamicType.GUILD_BOSS_TRIGGER then
		textSub = textTemplete--localizable.GuildBoss_Dynamic_tip_1
	elseif info.type == GuildDynamicType.GUILD_BOSS_START then
		textSub = textTemplete--localizable.GuildBoss_Dynamic_tip_2
	else
	...
	elseif info.type == GuildDynamicType.GUILD_KILL_BOSS then
		...
		if self.factionInfo then
			self.factionInfo.exp = self.factionInfo.exp or 0
			self.factionInfo.exp = tonumber(self.factionInfo.exp)
			self.factionInfo.exp = self.factionInfo.exp + tonumber(msgTab[5])
			self.factionInfo.boom = self.factionInfo.boom or 0
			self.factionInfo.boom = tonumber(self.factionInfo.boom)
	...
	elseif info.type == GuildDynamicType.UPDATE_GUILD_BANNER then		
		...
		self:pushFactionMsgToChat(strTemplete, math.floor(msgTab[1]),factionName)
	elseif info.type == GuildDynamicType.GUILD_BOSS_TRIGGER then
		local strTemplete = localizable.GuildBoss_Dynamic_tip_1
		self:pushFactionMsgToChat(strTemplete, math.floor(msgTab[1]),factionName)
	elseif info.type == GuildDynamicType.GUILD_BOSS_START then
		local strTemplete = localizable.GuildBoss_Dynamic_tip_2
		self:pushFactionMsgToChat(strTemplete, math.floor(msgTab[1]),factionName)
	end
end
...
function FactionManager:pushFactionMsgToChat(textSub, timestamp, factionName, playerInfo)
	...
	if playerInfo then
		...
		if playerInfo.starHeroRanking then
			msg.starHeroRanking = tonumber( playerInfo.starHeroRanking )
		end
		if playerInfo.ladderGrade then
			msg.ladderGrade = tonumber( playerInfo.ladderGrade )
		end
	end
...
function FactionManager:checkCanJoinFaction()
	...
	if not self.personalInfo.lastOutTime then
		self.personalInfo.lastOutTime = 0
	end
...
function FactionManager:timeFormat(time)
	local nowDate = GetCorrectDate("*t", time)
...
function FactionManager:isCanImpeach()
	...
	if (MainPlayer:getLevel() >= 30) then
		return true
	end
	return false
end
...
function FactionManager:canViewRedPointInMainLayer()
	...
	if GuildBossManager:checkRedPoint() then
		return true
	end
	
	if FactionDanFangManager:getRedPointInfo() then
		return true
	end
	return false
end
...
function FactionManager:exitAndClearData(clear)
	self.factionRecordTable = nil
	self.factionRecordTable = {}
	self.otherListInfo = nil
	self.otherListInfo = {}
...
function FactionManager:getRandomBannerInfo()
	local ChoseInfo = {}
	math.randomseed(MainPlayer:getNowtime())
...
-- 弹劾帮主
function FactionManager:requestTanHeAssistant(str)
	-- showLoading();
	print("~~~~~~~~~~~~~~~~~~",str)
	TFDirector:send(c2s.IMPEACHMENT, {str})
end

function FactionManager:requestTanHeBallot(statu)
	-- showLoading();
	TFDirector:send(c2s.VOTE_IMPEACHMENT, {statu})
end
...
--请求祭拜记录
function FactionManager:requestWorshipRecode()

	TFDirector:send(c2s.GET_WORSHIP_RECORD,{})
	showLoading();

end

--获取祭拜记录
function FactionManager:onWorshipRecodeSucc(event)
	hideLoading()
	-- 数据处理
	self.myWorshipRecode = {}
	local info = event.data
	if (info and info.infos ~= nil) then
		local recodeData = info.infos
		table.sort(recodeData,function(a,b)	return a.time>b.time end)
		local lastTimeData = GetCorrectDate("*t", recodeData[1].time /1000)
		local timeStr = lastTimeData.year..'-'..lastTimeData.month..'-'..lastTimeData.day
		self.myWorshipRecode[1] = {spilt = true,time = timeStr}
		for i = 1, #recodeData , 1 do 
			local timeData = GetCorrectDate("*t",recodeData[i].time /1000)
			if (timeData.yday ~= lastTimeData.yday or timeData.year ~= lastTimeData.year) then
				...
				table.insert(self.myWorshipRecode,{spilt = true,time = timeStr})
			end
			table.insert(self.myWorshipRecode,recodeData[i])
		end
	end
	self:openJiBaiJiLuLayer()
end

--打开祭拜记录页面
function FactionManager:openJiBaiJiLuLayer()
	AlertManager:addLayerByFile("lua.logic.faction.JiBaiJiLu",AlertManager.BLOCK_AND_GRAY,AlertManager.TWEEN_1)
    AlertManager:show()
end

--获取祭拜数据
function FactionManager:getWorshipRecodeInfo()
	return self.myWorshipRecode
end
--是否开启丹房
function FactionManager:isShowDanFang()
    local isShow = false
    -- 开启条件:帮派等级达到3级开启丹房。个人团队等级达到40级。服务器开关
    if (MainPlayer:getLevel() >= 40 and self.factionInfo.level >= 3 and 
    	MainPlayer:getServerSwitchStatue(ServerSwitchType.DanFang)) then
    	isShow = true
    end
    return isShow
end
function FactionManager:openTanHeLayer(str,status)
	local layer = AlertManager:addLayerByFile("lua.logic.faction.FactionTanHeLayer",AlertManager.BLOCK_AND_GRAY,AlertManager.TWEEN_1)
	if ( str and status ) then
		print("接受投票:onImpeachmentSucc",str,status)
		layer:setData(str,status)
	end
    AlertManager:show()
end
--接受投票
function FactionManager:onImpeachmentSucc( event )
	-- 2是投票状态
	self:openTanHeLayer(event.data.impeachmentDesc,2)
end
