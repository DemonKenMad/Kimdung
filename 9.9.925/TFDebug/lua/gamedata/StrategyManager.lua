...
function StrategyManager:getPower(isFromOnlySelfPower)

	if 1 then
		return AssistFightManager:getStrategyPower( LineUpType.LineUp_Main, isFromOnlySelfPower )
	end
...
function StrategyManager:updateFormation(formations)
	formations = formations or {}
	--下阵检测
	local isExchange = false
	for i=1,9 do
		local roleGmId = self.strategylist[i] or 0
		isExchange = false
		for j=1,9 do
			local roleGmId2 = formations[j] or 0
			if roleGmId == roleGmId2 then
				isExchange = true
				break;
			end
		end
	...
	for index,roleGmId in pairs(formations) do
		local role = CardRoleManager:getRoleByGmid(roleGmId)
		self:setRoleIndex(index,role)
	end
	self:updateAllFate(true)
	CardRoleManager.updateUsedCardList = true
	TFDirector:dispatchGlobalEventWith(StrategyManager.UPDATE_GENERRAL_LIST)
